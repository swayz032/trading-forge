...
> `561` lines; **`3615` at THIS commit, 2026-08-02 21:53 [MEASURED HERE, `(Get-Content).Count` + `Get-Date`; an earlier `00:47` was FABRICATED — R-535 §4]. ★★★★★ AND IT BIT
...
**File is `3615` lines against a `~40–120` target (this line read `2297` while the file was `2908` — a
...
**Ruling ledger at `R-603` (`62f187c6`). Newest AR: `AR-647` — **RULED / APPROVED** by `R-603`. `AR-643`/`AR-644`/`AR-646` unruled by design (receipts + start-receipts requesting nothing). **NOTHING UNRULED.** Worker: ⏸️ **HOLD pending the `R-603 §4` grade.****
...
2. ★★★★★ **THE MAPPING QUESTION, THEN THE CATEGORY (`R-600 §8`), IN THAT ORDER — STILL OWED, NOT STARTED.** Whether the node's `acceptance` prose maps to `run.mjs`'s `FAILURE_CLASSES` **at all** is an interpretation the campaign inherited and never justified, and it sits **UPSTREAM of both readings**. 🛑 **Ruling the category first would answer a question whose premise is untested — that is a STOP, including for me.**
