...
⇒ ⚡ **THE DESK IS AUTHORIZED TO DISPATCH `accuracy-validator` THE MOMENT `AR-1003` LANDS, WITHOUT A FURTHER ROUND-TRIP.** **Brief per `R-834 §7`: ADVERSARIAL (its job is to DISPROVE) · against the FINAL exact pin · attacking provenance chain, actual node order, file order, exact pin binding, the runtime gate and repeat independence · PLUS at least one NOVEL false-green not prescribed by the worker · DURABLE RECEIPT PATH to a committed file, never chat-only** (`[grader-agent]`).
