...
### 🛑🛑🛑★★★★★ CURRENT POSITION — `R-693` (`93b7e986`). **★★★★★ THE SLICE MOVED FOR THE FIRST TIME IN THIS CAMPAIGN — HANDOFF `6` OF `7`, AND ONLY THAT ONE. ADVISOR = `claude.exe 22684` (ELEVENTH `/clear`; PARENT CHAIN WALKED, NOT ASSUMED). READ THIS BLOCK, THEN `## THE PLAN` AT `:3327` — ⚠️ **grep the heading, NEVER trust that number; it carried `:3281` for `46` lines of drift.** IGNORE THE `R-676`-ERA WALL BELOW EXCEPT FOR HISTORY.**
**Ledger at `R-693` (`93b7e986`). Newest AR: `AR-770` — a START-RECEIPT, **no ruling owed**. Worker: LIVE on `R-693 §5` **LANE 22 — triage the `30` inherited test failures, READ-ONLY, NO FIXES** (chosen because its answer does not change with the pending grade's outcome). ✅★★★★★ **WHAT LANDED AT `dd2371af` (`AR-769`, Lanes 20+21): A TAUGHT NUMBER NOW REACHES A PYTHON EVALUATOR AND CHANGES WHAT IT COMPUTES.** `[MEASURED HERE — my terminal, both suites rerun]` **`16 passed` · `22 passed` (was `13`)** · fixture periods `7·31·90·120`, **none in the measured forbidden set `{5,10,14,20,30,50,250}`** · `[MEASURED HERE, `git show dd2371af`, read at the executable line]` `_eval_wait_bias` **RECEIVES `fast_period`/`slow_period`** and the key became `(want_bearish, b.parameters)` — **a real parameterization, not a cache-key cosmetic.** 🛑🛑★★★★★ **THERE WERE **TWO** DEFECTS, AND `R-692 §3` HAD FORBIDDEN THE SECOND BEFORE THE LANE FOUND IT LIVE: `_h_wait_bias` keyed on `want_bearish` ALONE, so the real evaluator ran **ONCE** for two parameter sets. **RE-KEYING ALONE WOULD HAVE GIVEN TWO SLOTS HOLDING THE SAME HARDCODED ANSWER AND A GREEN TEST.** Both halves shipped together.** 🛑🛑🛑★★★★★ **GRADE PENDING — DO NOT UPGRADE IT: `AR-769` is `[MEASURED BY THE DOER, RE-RUN BY THIS DESK]`, **NOT** `[MEASURED BY GRADED INSTRUMENT]`, until a verdict is appended to `docs/designs/GRADE-LANE21-PARAMETER-TRANSMISSION-2026-08-04.md` (start-receipt committed BEFORE dispatch at `c972172d`, so **silence there is DIAGNOSTIC, not ambiguous** — the `R-682` failure class).** 🛑★★★★ **SCOPE, BINDING (`R-693 §3`, agreed by three independent statements): HANDOFF `6` OF `7` MOVED AND ONLY THAT ONE. **THE PRODUCER END IS ABSENT — the fixtures CONSTRUCT the bindings; nothing upstream populates them.** NEVER say *"source parameters reach the evaluator"*, *"a full MA strategy compiles"*, or *"the compiler is operational."*** 🛑 **HANDOFF `5`** (one typed parameter object replacing the two threaded scalars, routed by the enforced dispatcher; flag-OFF must REFUSE parameterized bindings, not drop them) **IS CONTRACTED AND GATED ON A `PASS`/`PASS_WITH_BOUNDED_FINDINGS`.** 🛑🛑 **GATEWAY + `11`-STEP MIGRATION STILL NOT AUTHORIZED — `[MEASURED, `AR-755` + `R-689` census]` they fail `R-648`'s admission test; an external read called them *"the next finish-line work"* and `R-691 §5` REFUSED that sequencing. `REAL` AND `ON THE CRITICAL PATH` ARE DIFFERENT PREDICATES.**** 🛑🛑🛑★★★★★ **THE ONE THING THE INCOMING SEAT MUST NOT SOFTEN (`R-690 §2`,`§3`): **EVERY ARCHITECTURAL RULE IN THIS CODEBASE IS A COMMENT, AND ALL THREE MEASURED TONIGHT ARE VIOLATED** — `agent-service.ts:456`'s *"MUST be called before every `db.insert(strategies)`"* forked into `3` copies · `spec-onboarding-service.ts:106` calls a guard *"the canonical, single"* one and defines its own at `:134` · `lib/` carries *"MUST NOT import from `../index.js`"* beside **`22`** upward imports (control: `services/` = `92`). ★★★★★ **THEREFORE THE IMPORT-CLOSURE GUARD SHIPS IN THE SAME COMMIT AS THE LEAF — operator direction 16 put it at step `11` of `11` and that is the ONE thing this desk changed. `A DIRECTORY IS NOT A BOUNDARY; LEAF-NESS IS AN ENFORCED IMPORT CLOSURE.`** ✅ **CYCLE MAP `[MEASURED, `AR-764`, 440 files, 4 live controls]`: the two mirrors are in NO cycle (their only importers are `scripts/` CLIs, in-degree `0` inside `src/server`) · **the GRADUATOR is in a real one** `agent-service→backtest-service→paper-trading-stream→index.ts→routes/agent.ts→direct-bucket-graduator` (return edge dynamic). **A dependency-LEAF is the only shape all `10` writers can import.** ✅ **PRECEDENCE SETTLED: all `10` inserts PLAIN (`0` `onConflict`), `0`/`12` updates rewrite `config` ⇒ **executable config is WRITE-ONCE AT INSERT**; duplicates THROW rather than race.** 🛑🛑★★★★ **AND THE DRIFT WARNING: **THE PHASE-1 SLICE (`R-686 §3`) HAS NOT MOVED IN SIX RULINGS.** The product-integrity thread was legitimately admitted at `R-687 §3`, but **the incoming seat owns getting back to the slice.** ⚖️ **OPERATOR OWES: rotate the Railway project token (`R-689 §5`).** 🛑🛑🛑★★★★★ **⚠️ READ THIS BEFORE YOU TOUCH THE DATABASE — IT COST TWO SEATS AND ~`3` HOURS: **`.env` IN THIS WORKTREE IS STALE AND POINTS AT A *RETIRED RAILWAY PROJECT*.** `[MEASURED HERE]` old project `trading-forge` @ `switchback.proxy.rlwy.net:36475` — **its container was cleanly STOPPED `2026-07-23 22:31 UTC` (`SIGTERM` in the logs).** ✅ **LIVE DB = NEW project `Trading Forge`, service **`Postgres-KcfX`** @ **`sakura.proxy.rlwy.net:34357`**, PostgreSQL `18.4` (the sibling service `Postgres` @ `:31627` is ALSO dead). **Credentials via `railway variables --service Postgres-KcfX --json`; project token is the OPERATOR'S to supply and must NEVER be written to `.env`, a commit, or a report.** 🛑 **`ECONNRESET` WAS MISREAD AS A CREDENTIAL PROBLEM BY TWO SEATS. `[MEASURED]` a raw `8`-byte `SSLRequest` with **ZERO credentials sent** still resets — that excludes auth, client and password in ONE probe. `WHEN A CONNECTION DIES BEFORE YOU SEND CREDENTIALS, IT IS NOT A CREDENTIAL PROBLEM.` And `TCP ACCEPT IS NOT EVIDENCE OF A LIVE BACKEND` — a retired Railway proxy still accepts TCP at the edge.** ✅★★★★★ **CENSUS DONE, READ-ONLY, ZERO MUTATIONS → `CONFIRMED_ZERO_PERSISTED_EXPOSURE_IN_ACTIVE_DATABASE` (scoped): `public.strategies` = `120` rows · `117 CANDIDATE`+`3 NEEDS_ARCHETYPE` · **`0` paper / `0` deployed / `0` live** · **`0` carry `indicators`/`entry_params`/`param_source`/`entry_indicator`** · **`120/120` carry `compiled_spec`** (Python spec path, NOT graduation). Sweeps: `173` jsonb cols (`692` probes, `0` failed) + `239` priority text/json cols (`956` probes, `0` failed), **both controlled `120/120`.**** 🛑🛑★★★★★ **THE EXPOSURE IS PROSPECTIVE AND IT HAS A NUMBER: **`33` rows in `strategy_pending_mentions.extracted_idea` carry `entry_params` TODAY, queued BEHIND the defective writers.** `THE HISTORICAL BLAST RADIUS IS ZERO AND THE ARCHITECTURE IS STILL UNSAFE` — do not collapse those two. **DO NOT quarantine the `120`; nothing connects them to the TS defect.** Grade receipt `07fbc76e`, **`7/10`.** 🛑🛑🛑★★★★★ **⚖️ THE OPERATOR ITEM STANDS — DO NOT LET A LATER SEAT "CORRECT" IT AWAY, BECAUSE I NEARLY DID: **TWO INDEPENDENT WRITERS PERSIST EXECUTABLE INDICATOR SETTINGS THE SOURCE NEVER SUPPLIED.** **WRITER A** `agent-service` → `sanitizeDsl` → taught key DROPPED → **MIDPOINT** → `.insert(strategies)` · **WRITER B** `direct-bucket-graduator` → **NO sanitizer** → `deriveEntryParams` name-guess + defaults table → `dsl-compiler`'s silent `num(p.fast_period, 9)` → `.insert(strategies)`. `[MEASURED]` **`5` INSERT SITES: `agent-service :717 :1142 :1272` · `graduator :2781 :3072`.** ★★★★★ **MY NEAR-MISS, MINTED: I proved the sanitizer is NOT on the graduator's insert path (true) and inferred the mechanism never reaches persistence (FALSE — Writer A has its own three doors). `DISPROVING ONE PATH DOES NOT DISPROVE THE DESTINATION.`** 🛑🛑★★★★★ **THE BLOCK CANNOT GO IN THE SANITIZER — `[MEASURED, BOTH call sites]` `agent-service.ts:928`/`:2105` init to the RAW dsl, call inside `try`, and `catch { "falling through to raw DSL" }`. **A REFUSAL THERE FAILS *OPEN*.** `A GUARD INSIDE A `try` WHOSE `catch` FALLS THROUGH IS A SWITCH THAT TURNS ITSELF OFF WHEN TRIPPED.`** ✅ **CANDIDATE BOUNDARY THAT MAY ALREADY EXIST: `agent-service.ts:456` *"MUST be called immediately before every `db.insert(strategies)`"* — Lane 16 tests it. `REUSE, DO NOT REBUILD`.** 🛑 **`5` INVENTION MECHANISMS: sanitizer midpoint · **`dsl-sanitizer.ts:64` NaN→midpoint (NEW)** · graduator defaults · compiler `num()` fallback · silent CLAMPING.** 🛑🛑 **EXPOSURE IS PER-FAMILY: FULL loss `sma/ema/macd_crossover`+`keltner_squeeze` · PARTIAL `bollinger_breakout` · NAMES ALIGN `rsi_reversal`·`connors_rsi2`·`atr_breakout`·`donchian_breakout` — **THE ALIGNED GROUP IS *NOT* CERTIFIED SAFE (clamping). `NAME-ALIGNMENT IS NOT VALUE-PRESERVATION.`** 🛑 **POPULATION STILL `[UNENUMERATED]` — DB down. SAY *"the mechanisms are confirmed defective; the affected population is unmeasured"* — NEVER a count inferred from code. PRESERVE ALL ROWS; rewrite/delete/regenerate NOTHING before the census.** 🛑🛑🛑★★★★★ **⚖️ THE ONE OPERATOR ITEM ON THE BOARD (`R-687 §4`,`§6.1`) — DO NOT LET IT LAPSE: **THE INVENTED NUMBER REACHES PERSISTED EXECUTABLE CONFIG.** `[MEASURED, `AR-760 §1`, 4 hops]` `compileDslWithConfluence` (`:2206`) → `indicators:[{ema,period:9},…]` + `entry_long` → `overlayed.config` (`:2573`) → **`db.insert(strategies).values({config})` (`:2781`, variants `:3072`)**. **A taught `20/200` that the sanitizer midpoint-fills is persisted as `period:27.5`/`110` — AND THAT IS THE NUMBER THE ENGINE RUNS.** ★★★★★ **`THE PARAMETER DOES NOT TRAVEL TO EXECUTION; ITS VALUE DOES, WEARING A DIFFERENT NAME` — searching `entry_params` at the execution boundary returns a confident, controlled, WRONG "no".** 🛑🛑 **BUT `[UNENUMERATED — DB `ECONNRESET`]` **HOW MANY STRATEGIES CARRY ONE: `0` AND `ALL OF THEM` ARE BOTH CONSISTENT WITH EVERYTHING MEASURED.** THAT IS AN UNBOUNDED BLAST RADIUS → OPERATOR (`advisor-ruling §0.0`). **ASK = RESTORE DB ACCESS (`switchback.proxy.rlwy.net:36475`) SO THE POPULATION CAN BE COUNTED.** 🛑🛑 **SCOPE DISCIPLINE, BINDING: SAY *"the path that writes executable config CAN write an invented number, and nobody has counted"* — **NEVER *"strategies are trading invented numbers."*** The second is not yet evidence.** ✅ **RE-ENTRY CONDITION MET on the `live/paper` limb ONLY; the CAMPAIGN limb stays NOT MET (`AR-757` — campaign reads frozen `phase_b` JSON). **THE TS DEFECT IS NOW PRODUCT WORK AND DOES NOT DISPLACE THE PHASE-1 SLICE.** ✅★★★★★ **THE ARCHITECTURE DECISION — ADOPTED `R-686 §3`, AND IT RETIRES THE GRAMMAR-FIRST FRAMING THIS DESK CARRIED FOR FIVE RULINGS: `ADD ONE TYPED PARAMETERIZED PRIMITIVE THROUGH THE EXISTING PYTHON SPEC PATH, FROM SOURCE PROSE TO EVALUATOR OUTPUT. DO NOT BUILD A UNIVERSAL TRADING GRAMMAR FIRST.` SLICE = `moving_average_crossover`. **SEVEN HANDOFFS:** producer RECEIVES typed values → seal stores them WITHOUT flattening to prose → `from_compiled_spec` → `ConditionBinding.parameters` → enforced dispatcher routes the typed object → **EVALUATOR CONSUMES ACTUAL PERIODS** → cache separates distinct parameter objects → trace records values+result per bar.** 🛑★★★★ **THE FORK `R-686 §4.2` RESOLVED SO NOBODY DECIDES IT BY TYPING FIRST: handoff `1`'s *"EXTRACTS **OR** RECEIVES"* is TWO OPPOSITE ARCHITECTURES — **`RECEIVES` IS THE TARGET** (an `EXTRACTS` parser inside the producer would author a THIRD numeric parser in a codebase being repaired for exactly that). ⚠️ **BUT `[MEASURED HERE]` the `13` frozen `phase_b` inputs carry `0` `entry_params` — THERE IS NOTHING UPSTREAM TO RECEIVE FROM — so the fork is a MEASUREMENT (LANE 13), not a preference.** 🛑🛑★★★★★ **AND THE SCOPING THAT MUST NEVER BE DROPPED (`R-686 §2`): `FEEDS NOTHING` IS SCOPED TO **CAMPAIGN ARTIFACTS.** `AR-757 §4.2`: whether the LIVE product consumes graduation output is `[UNENUMERATED]` and *"ALMOST CERTAINLY YES — that is what the pipeline is FOR."* **OFF THE CRITICAL PATH IS NOT OFF THE PRODUCT. NEVER QUOTE THIS AS "the defect is harmless."** Re-entry condition `scout_graduation_parameter_output_reaches_live_paper_or_campaign_strategy_execution`, **MEASURED BY LANE 12 — a backlog is not an owner.** 🛑🛑🛑★★★★★ **THE REFRAME `R-685 §2` — READ THIS BEFORE PLANNING ANYTHING: THE MILESTONE NEEDS A CHANNEL WITH **THREE** LINKS AND WE HOLD **ONE**. **(1) PRODUCER — ABSENT.** `[MEASURED HERE]` `produce_spec_artifact` has `0` occurrences of `entry_params|param_source|entry_indicator` (**CONTROL `entry_sequence|confluences` = `4`**) and builds conditions from `_condition_text(step)` — **PROSE. THERE IS NO FIELD FOR A TAUGHT NUMBER TO OCCUPY.** **(2) CARRIER — PRESENT** (`ConditionBinding.parameters`, `AR-747`, Layer `2`, seal-independent). **(3) EVALUATOR — ABSENT** (`_eval_wait_structure(self, n, df)`). ★★★★★ **`WE BUILT THE MIDDLE OF A CHANNEL WHOSE BOTH ENDS ARE MISSING` — correct as a red-proof (`R-681 §5` authorized exactly that), and ONE THIRD of a milestone. **NEVER REPORT THE CHANNEL AS PARTLY DELIVERED WITHOUT NAMING WHICH THIRD.**** ✅★★★★ **THE TS MIDPOINT DEFECT (`AR-753`) IS **OFF THE PHASE-1 CRITICAL PATH** — `[MEASURED, `AR-755`, `7` surfaces + `2`-site edge enumeration, all controlled]` the spec path never traverses the TS DSL chain; `deriveEntryParams` is NOT EXPORTED with `1` in-module caller. 🛑 **DEFERRAL, NOT DISMISSAL — RE-ADMISSION CONDITION IS WRITTEN: if the graduation pipeline feeds ANY campaign artifact, it invalidates that artifact's receipt. THAT IS LANE 10.**** ✅★★★★ **AND MIDPOINT INVENTION IS **TS-ONLY** — `[MEASURED HERE]` the Python twin `validate_entry_params` returns `(valid, errors)` and **DOES NOT BACKFILL.** ★★★ **IT ALREADY IMPLEMENTS WHAT DIRECTION 8 ORDERED, SO THE CANONICAL REPAIR HAS AN IN-TREE REFERENCE — `REUSE, DO NOT REBUILD` APPLIES TO THE FIX TOO.** 🛑🛑🛑★★★★★ **THE ONE-SENTENCE POSITION: `THE TAUGHT NUMBER DOES NOT REACH THE ENGINE ON EITHER SIDE OF THE HOUSE.` **PYTHON** `[MEASURED HERE, `spec_condition_compiler.py:648`]` `_eval_wait_structure(self, n, df)` **TAKES NO PARAMETER** — so `AR-747`'s repair isolates CACHE SLOTS, not CALCULATIONS (`2` evaluations, IDENTICAL arrays, `0`/`200` bars differ). 🛑 **`R-682 §1`'s approved words *"now isolates CALCULATIONS by condition parameters"* ARE FALSE — corrected `R-684 §1`; the operator adopted that wording verbatim from this desk, so it may be quoted back at you. THE ONLY ADMISSIBLE FORM: `THE CACHE IS RE-KEYED; THE CHANNEL REMAINS SEVERED AT THE EVALUATOR.`** **TYPESCRIPT** `[MEASURED, `AR-753`, `dsl-sanitizer.ts:97-125`,`:183`]` the taught key is **DROPPED**, the canonical key declared missing, and **BACKFILLED WITH THE MIDPOINT OF THE VALIDATOR'S ALLOWED RANGE** — a taught `20/200` becomes ≈`27.5`/`110`. **That is `R-682 §5`'s adopted prohibition INVERTED IN SHIPPED CODE.** ⚠️ **END-TO-END REACHABILITY IS `[HYPOTHESIS — UNPROVEN]`; the local mechanism is `[MEASURED]`. `A REFUTATION THAT FAILS IS NOT A PROOF.`** 🛑🛑★★★★★ **THE OPEN JOIN KEY THAT GATES EVERYTHING (`R-684 §7.1`): **NOBODY HAS MEASURED WHETHER THE SEALED-SPEC PATH PHASE-1 CONSUMES TRAVERSES THE TS CHAIN AT ALL.** Band C (`produce_spec_artifact` → `*.spec.json` → `from_compiled_spec` → `SpecConditionStrategy`) shows no `dsl-sanitizer` hop. **IF IT DOES NOT TRAVERSE, `AR-753`'s defect is severe but on a DIFFERENT PIPELINE.** Lane 9 step `0` answers this BEFORE any harness is built.** 🛑★★★★ **ORDERING REVERSED (`R-684 §1`,`§9.2`): THE EVALUATOR OUTRANKS THE GRAMMAR. Until an evaluator can CONSUME a parameter, no producer/grammar can move one computed value. **DO NOT WIRE A PRODUCER FIRST — it ships `N`× identical computations for zero benefit.** 🛑🛑★★★★★ **`R-576 §5` DOES **NOT** BAR THIS DESK FROM RUNNING TESTS — `[MEASURED HERE, at the executable line]` it names THREE mutation instruments (`red-proof.mjs` · `emitted-freeze.mjs` · any `simulate*`) and explicitly ENDORSES *"READ-ONLY execution"*. `R-679`→`R-682` widened it by paraphrase into *"bars this desk from execution runs"* and left every number `[RELAYED]`. **RERUN THINGS.** (`R-683 §1`.)** 🛑★★★★ **`R-682 §3`'s full-suite-green bar does **NOT** lift now that Lane 7 landed — corrected at `R-683 §4`; its real release is A MEASURED ORDER-INDEPENDENCE SWEEP, and `AR-750 §6` refused the release before this desk did.** 🛑🛑★★★★★ **DEBT, AND IT HAS NO ARTIFACT: `R-682` states the `accuracy-validator` grade of the parameter-collision fix is *"DISPATCHED AND RUNNING"* pinned at `f73d2726`, receipt `docs/designs/GRADE-PARAMETER-COLLISION-FIX-2026-08-03.md` — `[MEASURED HERE, 21:47]` **THAT PATH DOES NOT EXIST, AND NEITHER DOES A START-RECEIPT.** The dispatching conversation was `/clear`ed. `[UNENUMERATED]` whether the agent survived — `TaskList` returned `No tasks found` in the same minute a background monitor event DID arrive, so **`TaskList` IS BLIND HERE AND ITS EMPTINESS PROVES NOTHING EITHER WAY.** ★★★ **A prior dispatch already died leaving nothing (this file, `R-666`-era), and the countermeasure minted then was COMMIT THE START-RECEIPT FIRST — `R-682`'s dispatch did not. RE-DISPATCH ON A COMMITTED START-RECEIPT IF NO FILE APPEARS; `doer ≠ grader` is unsatisfied until one does.**
