...
8. **`parameter_status` MUST DISTINGUISH `TAUGHT` / `ASSUMED` / `REFUSED`** — `AR-737 §4` measured an educator saying *"it's not the exponential"*, an explicit REFUSAL that `type: SMA` alone does not record. **Missing required parameter ⇒ `BLOCK: missing_source_parameter`; ambiguous ⇒ `BLOCK: unresolved_source_ambiguity`; engine defaults may NEVER substitute.**
