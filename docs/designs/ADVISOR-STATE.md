...
✅★★★★★ **FRESH ADVISOR SEAT since `12:07`. WORKER SEAT IS LIVE AND MID-BATCH — `AR-606` start-receipt, `AR-607` closed item `(5)`. FAN-IN `1 / 4`.** **Remaining, worker's own order: `(2)` `F-3` → `(3)` `Proxy` → ALL corpus rows in ONE pin dance (folding in `EXEMPT_EXPORTS` DELETE + `HISTORICAL_RENAMES` PIN + the `§6.3` enumeration, all ruled in `R-570 §4`).**
