# CANONICAL ADVISOR HANDOVER — **CURRENT AT `R-753`, 2026-08-09**

> 🛑🛑🛑★★★★★ **READ THIS BLOCK FIRST. IT SUPERSEDES EVERY BLOCK BELOW, INCLUDING THE TWO THAT CALL THEMSELVES CURRENT AT `R-752` AND `R-750`.**
...
> ⏳🛑 **THE ONE THING THAT CAN SILENTLY GO MISSING — READ BEFORE ANYTHING ELSE: AN `accuracy-validator` GRADE IS IN FLIGHT, DISPATCHED BY THE RETIRED ADVISOR SESSION.** Pinned SHA **`16224ef5`**, isolated worktree `wt-grade-d8`, items `(A)–(K)`, receipt → **`docs/designs/GRADE-TRIGGER-SAFETY-2026-08-09.md`**, **written UNCOMMITTED — THE DESK COMMITS IT** (`[precommit-stash]`).
...
> ⚡ **WHERE THE MONEY PATH IS:** the refusal is now terminal **end-to-end** — Python `main()` **and** the TypeScript service, which persists `status="refused"` instead of `completed` (`D-8`, `16224ef5`). ✅ **Desk-measured release predicates: Python `65 passed` · TS control C `20 passed` · TS `D-8` controls `17 passed` · `103`-manifest membership `NEW 0 / GONE 0`, both ordered `6B` REDs present, with a TAMPERED negative control returning `NEW 1 / GONE 1`.**
...
> ⚠️ **MONITORS: ONE ruling-ear authorized until the grade receipt lands, then DISARM** (`R-753 §5`). **Arm nothing else.** ⚖️ **The reversal is `[CORROBORATED]` by two relays, NOT heard first-hand by the desk — say so if you rely on it.**
