# Mode A/B (G4) Validity Block — 2026-07-11

**STATUS: READ CERTIFIED (2026-07-12). The directional read was taken ONCE (§READ) on the full N=25 informative set, independently re-verified (numbers clean), narrative corrected per grader. TERMINAL RESULT: the overlay is LOSS-REDUCING (near-mechanical, Pearson +0.83) on this negative-edge corpus — overturns corpus-v2 'overlay HURTS' as a missing-exits artifact; does NOT establish edge-accretion on profitable strategies (out of scope). Only remaining step: consumption decision (overlay frozen-status) with operator. Full sequence below: §RULING → §VERIFY → §NREAD → §READ.**

**Scope line (Law 7):** corpus = specbridge-120 → Mode A/B 78-spec subset · battery = Mode A/B overlay-OFF(A)/ON(B), full WF+CPCV+PBO+DSR · engine = G4 (`backtester.py` 8501 lines, sha256 `79570c4f..`, mtime 2026-07-09T17:53:08, HEAD `00c26184`) · snapshot = data_cache ratio_adj parquets stamped below · effective-N = §C8.

---

## §0 — Pre-commitment (Law 4, registered PRE-NUMBER, 2026-07-11 ~13:15 EDT)

Registered before any directional number exists, so the evening read cannot relitigate it:

> If spec 2 (`589ec512`) — or a restarted spec 1 (`ca5564d8`) — **cannot complete at solo (the safest possible config, MAX_CONCURRENT=1) on this tower**, it is excluded as **resource-infeasible**, documented with its runtime evidence (elapsed, OOM/freeze signature), effective-N adjusts, and the read proceeds on what completed. This is a **forced scope with a stated cause, decided before any number exists** — categorically distinct from choosing to read early. Solo has held all day, so this branch is not expected to fire.

---

## §R — Fork-resolution criteria + locked read order (Law 4, pre-committed 2026-07-11 ~14:30 EDT, PRE-NUMBER)

Ratified as the criteria for the evening read, applied mechanically, BEFORE the two readings' arms are seen.

**Record discipline on the prior "7".** The earlier classification ("7 pairs, OR-fix, terminal") was ratified on the then-current instrument and is NOT silently overwritten. The record carries the sequence: **7-classified → this block exposed the degenerate G0 `trade_count` counter → population revised to ~12 candidates → re-classification at the read under the same standards.** The mechanism ratified (OR-fix unconditional direction-correctness components; trigger-field evidence) **STANDS as a verified cause**; the population it covers is **REOPENED**. No classification extends by momentum — every candidate flip earns its label under the same per-pair standard (trigger fields wait_bias/confirm/direction present in its OWN config), or it is unexplained. Per the frozen 0.5 pre-reg: **any flip that does not classify = FULL STOP, not a scope note.**

**Fork-resolution criteria** (instrument choice: G0 daily_pnls activity vs G0 trade_count):
1. **Result-independence** — instrument chosen on instrument-quality grounds alone, before anyone sees which reading flatters which pair.
2. **Economic grounding beats derived counters** — a reading anchored in real events (nonzero-P&L days, actual fills) outranks the `trade_count` field now known to produce exactly-8 artifacts across 44 specs.
3. **Uniform application** — one instrument for all pairs; no per-pair instrument shopping.
4. **Tie-breaker: fail toward suspicion** — if quality grounds don't decide, take the reading generating MORE candidate flips to classify, never fewer (same failure-direction as the F-2 matcher).

**Pre-number resolution (result-independent, recorded before the verdict):** criteria 1+2 select the **economic daily_pnls activity instrument** over the degenerate trade_count counter. Selection made now, on instrument-quality grounds, blind to the arms. Per-pair *classification* waits for the full population.

**Instrument distinction (preserve exactly):** daily_pnls is the *activity* instrument — it answers "did G0 trade?" honestly **even when the trades themselves were bug-fires**; *why* it traded and *why* it stopped belong to the classification step. The instrument does not need the trades to be legitimate, only the record of them to be real.

**Locked evening read order** (one sitting, one read, when both straggler rows have appended):
1. Resolve the fork by the criteria above (instrument = activity). *Done pre-number.*
2. **Symmetric flip sweep — BOTH directions** (anomaly criterion is symmetric): down-flips (§C1) AND gain-flips (§C2b). *Earlier "0 upward anomalies" was on the degenerate counter → VOID; activity sweep replaces it.*
3. **Door-1 scoping (RULED, §C2c):** hard run-failures ONLY for the flip taxonomy; degenerate-fingerprint = cross-generation SCOPE MARKER, not exclusion; arm-identity re-check DONE (44/44 arms differ → 'overlay-invariant' dropped from the fingerprint).
4. Classify every candidate flip (both signs) per-pair under the CLOSED taxonomy. Neither-trigger-yet-flipped → **FULL STOP** (mesh intact).
5. Verify 331fe15a (§V) — now covers 2/3 valid baselines (331fe15a + MES sibling 5e5803d1).
6. THEN and only then: direction + classifiability on the full 78 — once (Law 3), with EVERY cross-generation magnitude/agreement claim SCOPED to the valid-baseline pairs (§C2c: `b2deddcb` clean pair + final-population additions).

**Closed classification taxonomy — exactly ONE label per candidate; no fourth label, no blend, no "probably":**
1. **OR-fix class** — per-pair trigger-field evidence (an unconditional component: wait_bias forced-direction OR confirm direction-discard) PLUS the direction-sensitivity that makes the mechanism bite. Same mechanism, either sign.
2. **Fidelity-artifact class** — verified per a §V-style spec (331fe15a's unsatisfiable-AND = type specimen); each proven from the compiled conditions, never asserted.
3. **Unexplained** — FULL STOP per the frozen 0.5 pre-reg. Does not soften near a verdict.

**Component-sign predictions (pre-registered before compiled evidence is read).** The OR-fix has TWO unconditional components with DIFFERENT sign predictions:
- **Component (i)** `_eval_wait_bias` forced-bullish — can only OVER-fire longs in G0 and suppress direction-constrained shorts. **Structurally CANNOT explain a long spec going G0-dead→G4-alive.**
- **Component (ii)** confirmation direction-discarding blend — suppresses EITHER direction (a long whose entry needed a confirmation the blend could never produce is unsatisfiable in G0, freed by the fix: 0→1200+ trades is exactly that shape).
- **Falsifiable prediction:** an OR-fix-class GAIN-flip config should carry **confirm:True** (with or without wait_bias). confirm:True → component-(ii) OR-fix class (two components, two signs, one fix). NEITHER trigger → prediction FALSIFIES → routes to fidelity-artifact-or-FULL-STOP on its compiled conditions. The sitting reads a prediction, not a rationalization.

Relay the fork resolution and each flip classification as they are made.

## §AM — FORMAL AMENDMENT to the frozen flip-taxonomy (operator-ruled live, 2026-07-11 night sitting, PRE-NUMBER)

**Executed in the frozen rule's OWN order** (`re-baseline-preregistration-2026-07-07.md` ANOMALY criterion 1: "any (d) or predicate-fail = FULL STOP, contradicting-evidence protocol, reference reopens" — FULL STOP is the rule's amendment door, not exam-death):
1. **Strict reading FIRED.** As frozen, (a)/(b)/(c) = the Defect-10 removal taxonomy (removed / occupancy-freed / timing-shifted). The 4 gain-flips + the fidelity candidates do NOT map → under the strictest reading they are (d). No softening. FULL STOP executed as written.
2. **Contradicting-evidence protocol ran on evidence entirely PRE-NUMBER.** Finding: the frozen taxonomy is incomplete by omission — frozen 2026-07-07 contemplating Defect-10 as the only legitimate count-mover, blind to a SECOND count-moving fix already in the engine at freeze time: the OR-branches fix's unconditional direction components, invisible then because they shipped unreceipted behind a flag (the flag-law incident, discovered 07-10, mechanism-verified to trigger fields on a tested spec, receipted in the CCR artifacts — all before tonight, all before any number). The pre-reg didn't soften; the world got more known through the campaign's own tripwires.
3. **STRUCTURAL AMENDMENT — the taxonomy extends by two homes, strict admission:**
   - **(e) Receipted-fix class:** a flip explained by a landed, receipted engine fix whose count-moving mechanism is independently verified — admitted per-pair on trigger-field evidence ONLY, both signs (gain = same fix, opposite polarity = component-(ii)). NO pair enters (e) on momentum, family resemblance, or the sibling's evidence.
   - **(f) Spec-defect class (fidelity-artifact):** a flip explained by a defect in the SPEC, not the engine — admissible ONLY after per-spec §V verification passes. Until §V passes, the label is unassignable and the pair sits at (d)-PENDING. §V fails → the pair IS (d) → FULL STOP, for real.
   - **(d)** retains everything else; the mesh is untouched. The two ambiguous gain-flips (avoiding_two_mistakes pair, wait_bias-not-confirm) CANNOT take (e) on wait_bias alone (component-(i) cannot explain long-side gains) → they earn (f) through their own compiled-conditions §V, or they are (d).
4. **"Reference reopens" SATISFIED:** the door-1 ruling already re-derived the comparison basis (degenerate-fingerprint scope marker collapsed the cross-generation surface to the 3 valid-baseline pairs).
5. **Amendment audit trail (Law-4 compliance):** (i) discovery-chain = flag-law incident + trigger-field verification + CCR receipts, all pre-number; (ii) result-independence = derivable entirely from code archaeology predating the battery's completion; (iii) operator present and ruling live; (iv) DIRECTION CHECK = the amendment decides halt-vs-proceed, NOT what the direction says (informative surface unchanged), and it keeps a LIVE PATH TO ITS OWN FAILURE — if §V fails on the long_opportunities siblings, 2 of 3 valid-baseline pairs go to (d) and the FULL STOP takes the sitting. An amendment that can still say no is a rule, not a rationalization.

**Per-pair classification under the amended taxonomy (mechanical rails, mesh armed):**
- **22 down-flips → (e)** receipted-fix (each carries OR-fix trigger evidence — wait_bias and/or or_branch and/or confirm, verified per-pair; NONE trigger-less → 0 mechanical (d)).
- **2 gain-flips → (e)** component-(ii): `67d67ba0`, `f3babbdd` (confirm=True).
- **4 → (f)-PENDING §V** (blind adjudicator gates (f)-or-(d)): `331fe15a` + `5e5803d1` (long_opportunities siblings, valid-baseline — §V gates 2/3 of the comparison surface), `ca5564d8` + `589ec512` (avoiding_two_mistakes siblings, degenerate-G0, (e)-barred).
- **`b2deddcb`** → (e)-candidate component-(ii) (confirm=True) for the +231/+48% count-classification duty; else occupancy-freed(a/b/c) or (d) [ANOMALY criterion-2].
- **`ae7a2560`** → no-baseline, exits the flip taxonomy (Defect-4 reconciliation).

## §SV — §V blind-adjudication results (fresh-context adjudicator, blind to tally/outcomes/consequences)

Adjudicator rulings: **DEFECT · DEFECT · FAITHFUL · FAITHFUL.** Transcript fetch VPN-blocked; adjudicator obtained source title via WebFetch (`HfZTCZTDfWk` = *"The PROVEN 4-Hour Trading Strategy 85% Win Rate You Can Start Today"* — a bidirectional SMA-crossover) + the specs' own embedded evidence quotes; all 4 decidable on compiled structure alone (no INDETERMINATE).

- **`331fe15a` + `5e5803d1` (long_opportunities siblings) → DEFECT → (f) FIDELITY-ARTIFACT.** `and_group0` conflates `5-SMA-cross-above-50 + long` AND `5-SMA-cross-below-50 + short` (mutually-exclusive, opposite-direction triggers merged into one simultaneous AND) + a second instance (`buy-zone` AND `sell-zone`); the `entry_trigger_id` points INSIDE the dead group; source describes alternative directional branches the compiler mashed into an unsatisfiable AND. Confirmed byte-present in BOTH siblings independently. **G4's zero/near-zero is the honest reading of a mis-extracted spec.** → the 2 valid-baseline siblings are HOMED at (f); the valid-baseline comparison surface is intact (b2deddcb + 331fe15a(f) + 5e5803d1(f)).
- **NAMED H1 FINDING (the fidelity payoff):** *direction-conflated AND-groups* — opposite-direction (long/short) triggers merged into a single simultaneous AND requirement instead of split into directional OR-branches — are an **extraction/compilation defect class the Phase-1 fidelity instrument must detect at compile time.** A spec that demands mutually-exclusive conditions at once is a compile-time absurdity a fidelity certificate should catch before an engine ever sees it. (Also flags a cross-instrument note: the source is EUR/USD forex mapped onto MNQ/MES.)
- **`ca5564d8` + `589ec512` (avoiding_two_mistakes siblings) → FAITHFUL → §V does NOT pass.** Per the §AM amendment's explicit pre-commitment ("earn (f) through their own compiled-conditions verification **or they are (d)**"), §V-FAITHFUL means (f) fails → **(d) → FULL STOP fires on these two pairs.**

### ⛔ PARKED (ABSOLUTE parking rule) — FULL-STOP SCOPE, a tension between two operator rulings that §V-FAITHFUL exposed
The `avoiding_two_mistakes` pair sits at the intersection of two rulings that give OPPOSITE answers, visible only now that §V returned FAITHFUL:
- **§AM amendment** → "(f) or (d)"; §V-FAITHFUL → **(d) → FULL STOP** (frozen rule: any (d) = FULL STOP).
- **Door-1 ruling (§C2c)** → the exactly-8 degenerate fingerprint is a scope marker; these pairs' G0 baseline is a BROKEN instrument (arm-identity confirmed the counter lied). A "flip" measured against a broken G0 baseline is a comparison the degenerate baseline cannot support → **no-reliable-baseline → scoped out (ae7a2560-class), NOT (d).**
Both are the operator's own pre-number rulings; they conflict ONLY on this degenerate-G0 gain-flip pair. **NOT resolved on the spot (night, doer seat).** Note: this pair is NON-valid-baseline (door-1 already excluded it from cross-generation claims), so the direction read's INPUTS (the valid-baseline surface) are unaffected either way — the open question is strictly whether the frozen "any (d) = FULL STOP" fires on a degenerate-G0 pair door-1 already scoped out.
**Direction read HELD pending operator reconciliation of the FULL-STOP scope.**

### §WS — Window-start signature check (pre-registered days ago for the exactly-8 class; bidirectional tie-breaker, run at the sitting)

Pre-registered signature: forced-bullish WAIT_BIAS resolves the wait instantly → G0 fires sit at the first evaluable bar of EACH WF window, once per window, 8 windows. Mechanism refinement (stated before the check, so it could die): component-(i) is a signal-stream REPLACER — stereotyped window-start bug-fires MASK the real stream; both flip signs = one mechanism (G0 prints ~8 for everyone; G4 evaluates honestly, revealing 0 for down-flips / 1200+ for these gain-flips) — also explaining why 44 heterogeneous specs all printed exactly 8. Pure arithmetic on G0 daily_pnls nonzero-day positions vs the 8-window grid (8 windows confirmed from the WF logs).

- **CORPUS (branch 3) — CONFIRMED.** 44 exactly-8 specs: **62% of all fires within the first 5% of a WF window** (random ≈ 5% → ~12× enrichment). Down-flip exemplars sit on k/8 boundaries: `4be0db1d` frac positions [0.002, 0.125, 0.25, 0.375, 0.502, 0.874]; `543874f8` 10 fires across 7 windows all on k/8. **The 22 down-flip (e) classifications now rest on a corpus-confirmed signature, not a mechanism story alone.**
- **avoiding_two_mistakes pair — NOT confirmed.** `ca5564d8` and `589ec512` each show **exactly 1 fire, at index 0 (frac 0.0) — window 1 ONLY**; windows 2-8 starts empty. Does NOT show the pre-registered "each window / eight windows" multi-window pattern the down-flips demonstrate; and the refinement's prediction (gain-pair mask == down-flip mask) is FALSIFIED — their G0 is a single tiny blip ($1.62 / −$8.74), not the 8-at-8 stereotype.

**BRANCH DETERMINATION → branch 2 → (d) → FULL STOP (operator-CONFIRMED, 2026-07-12 ~02:00).** Precise framing (operator refinement, frozen): the pair's signature did NOT *fail* — it is **UNTESTABLE at n=1** (one fire cannot cluster; the clustering test is LOW_POWER on a single point, treated as an undersized stratum per campaign standard). What was cleanly **FALSIFIED** is the stated-to-die mechanism prediction (their G0 mask == the down-flips' multi-window stereotype — it doesn't; lone bar-0 blip vs crisp k/8 boundaries). **Falsified prediction + untestable signature + criterion-4 (suspicion default) = cannot admit (e) on evidence = (d) → FULL STOP.** We did NOT disprove masking for this pair — we proved we cannot prove it, which is what the suspicion default is for. Boundary-honesty note: the lone fire IS at index-0/window-1, a coincidence that could have rationalized branch 1; reading it as "the signature" after the multi-window version returned would be post-hoc widening — Law 4 exists for 2am. **Direction read HALTS; not taken.** (Branch 3 — the 22 down-flip (e)'s corpus-validated at 12x enrichment — STANDS regardless.)

**What tonight produced (a stop this honest is a RESULT, not a failure):** corpus signature CONFIRMED (12x) converting 22 (e)'s from mechanism-story to corpus-validated evidence; §V complete with banked H1 finding (direction-conflated AND-groups = named Phase-1 extraction-defect class); valid-baseline surface clean; the amendment proved itself a rule by saying no. The exam caught a real unknown TWO STEPS before the verdict could be contaminated by it.

**RE-ENTRY CONDITION (pre-committed 2026-07-12, evidence-gated NOT clock-gated):** mechanism named with run-level evidence (G0-era engine confession run) + per-pair classification updated under existing admission standards + INDEPENDENT verification of the naming → THEN the read (b2deddcb count-classification → direction ONCE → independent re-verify → certify). Not before. If any G4-side residue surfaces → bigger stop, everything holds.

### §CENSUS — the suppressor is 1m-timeframe-specific (Phase-C, two-path rule fired)

Pre-registered prediction (stated to die): the 6 lone-index-0 specs share wait_bias + a non-rearming state-machine. **FALSIFIED** — `67d67ba0`/`f3babbdd` carry confirm, not wait_bias. The two-path rule (census vs mechanism-hypothesis) caught it. **The real commonality: PERFECT 1m equivalence.** lone-index-0 by timeframe = {1m: 6}; ALL 6 clean-G0 1m specs are lone-index-0; ZERO non-1m specs are. The fingerprint IS the 1-minute timeframe.
- **Reframed mechanism (candidate, confession-run confirms):** the G0-era engine (≤ `48d3cc06`, 2026-07-04, pre-OR-fix) had a **1m-data-processing degeneracy** — every 1m spec produced tc=8 + a single bar-0 P&L blip + then dead; the G4 engine processes 1m honestly (1,200–1,538 trades). So the 1m 'gain-flips' are a **G0-side, 1m-contained engine limitation later fixed** — not a wait_bias masking, not an unmodeled cross-timeframe suppressor.
- **Threat assessment (Phase-D PRELIMINARY, census-level):** the degeneracy is **CONTAINED to 1m** (perfect equivalence). The valid-baseline surface — `b2deddcb` (15m), `331fe15a`/`5e5803d1` (4h) — is **NON-1m → UNAFFECTED.** The 23 informative pairs the STOP protects are not threatened by a 1m-only suppressor. This substantially de-risks the STOP, but does NOT lift it: re-entry still requires the mechanism NAMED by a confession run + independently verified.
- **Confession run (Phase-B) target:** G0-era SHA `48d3cc06`; run one 1m spec (`ca5564d8`) on a SHORT window with logging on the 1m data-load / window / entry-timing path; **log bars-PROCESSED-PER-WINDOW for all 8 WF windows** (the fingerprint — which window the 1m data dies in), plus the G4 contrast (all 8 windows on full bars = behavioral proof the choke is G0-side-only, the actual stop-lifter). Dispatched to fresh capacity.

**Two diseases now cleanly separated (do NOT conflate):** (1) NON-1m multi-window once-per-window bug-fires (k/8 signature, corpus-validated 12x — the (e) class's evidence); (2) 1m bar-0-then-dead choke (the census cut). Different mechanisms, different signatures.

**CHECK #1 (operator-mandated, done) — the (e) class had 1m contamination, now corrected:** 2 of the 22 (e) down-flips are 1m — `c029e162` + `81002a32` (`manipulation_trade` siblings) — admitted (e) on trigger fields but they showed the 1m bar-0 choke, NOT the multi-window signature. **Wrong disease. (e) class corrects 22 → 20 (non-1m only).** All SIX clean-G0 1m specs now sit as ONE population awaiting the confession: 4 gain-flips (`67d67ba0`/`f3babbdd`/`ca5564d8`/`589ec512`) + 2 mis-(e) down-flips (`c029e162`/`81002a32`) = exactly the lone-index-0 census set.

**Three-way convergence (strongest evidence form the campaign recognizes):** the SAME six 1m specs are identified independently by three unrelated derivations — the activity instrument (G0-dead → G4-traded), the lone-index-0 census fingerprint (1 P&L blip at index 0), and the timeframe cut (all clean-G0 1m). Convergence from independent paths → the 1m-choke hypothesis enters the confession run already three-way corroborated.

**PRE-COMMITMENT (operator, pre-confession, on record):** IF the confession names the 1m choke with run-level evidence + independent verification → **every 1m G0 row becomes a door-1 HARD-failure exclusion (named-mechanism no-reliable-baseline, ae7a2560's class).** The 6 1m specs exit the flip taxonomy LEGITIMATELY — a named, verified run-failure means there was never a baseline to flip from; no (e)/(f)/(d) needed. The mesh's integrity survives: the exclusion rests on an OBSERVED mechanism, not convenience — the exact line refused ~3h ago when the same destination was offered on inference alone (door-1-scopes-it-out lean). Same answer, now earnable honestly. **If the confession does NOT name it → the 6 stay (d) → FULL STOP holds.**

**RE-ENTRY CHECKLIST (updated, evidence-gated):** name mechanism w/ run evidence → INDEPENDENT verification of the naming → re-route all 6 1m specs to named-no-baseline (exit taxonomy) → b2deddcb count-classification → direction ONCE → independent re-verification → certify. Not before. Any G4-side residue → bigger stop.

### §CONFESS — confession run result: mechanism NAMED, 1m-containment REFUTED, BIGGER STOP (2026-07-12 ~03:00)

Fresh-context confession agent, G0-era engine `48d3cc06`, head-to-head vs G4 HEAD on identical inputs. **HIGH confidence, direct behavioral reproduction.**
- **MECHANISM NAMED:** the G0-era class path (`run_class_backtest`) NEVER injected exit signals before vectorbt. Compiler sets exits all-False ('framework-owned'); pre-fix class path had no compensating stop/time-stop call. All-False exits → `vbt.Portfolio.from_signals` holds ONE position per WF window → **trade_count = 8 = n_splits.** Head-to-head window-1, identical 13,973 raw entries → 242 after daily-cap: **G0 = 1 trade (drop 100%), G4 = 211 trades (drop 13%)** — the ONLY difference is the exits array (G4 class path calls `_apply_dsl_stop_loss_and_time_stop` E.3+E.5 15:55-ET time-stop; G0 does not). Windows 2-8 are FULL (142,914-bar frames, ~14k entries each) — NOT empty; the 2.7h runtime was real compute. Fixed by **landed+receipted commit `34fabc6d`** (2026-07-06, 'class-(a) defects fixed in run_class_backtest'), 2 days after the G0 baseline (2026-07-04).
- **1m-CONTAINMENT REFUTED (census hypothesis dead, two-path rule):** trade_count=8 is CORPUS-WIDE across ALL timeframes (5m:20, 15m:8, 1h:4, 30m:2, 4h:4, 1m:6). The '1m bar-0 blip' is the cosmetic endpoint of a monotone nonzero-P&L-day gradient (1m=1 … 4h=9-10; flips to 7-8 in Mode B), NOT a distinct 1m mechanism.
- **DECISIVE CHECK (execution_path):** ALL 47 clean G0 specs used execution_path=`class` — INCLUDING the valid-baseline surface (`b2deddcb`=485, `331fe15a`=93, `5e5803d1`=82, all class, all wfe=None). The valid-baseline pairs are on the SAME missing-exits-defective path; their G0 counts are artifacts of held-position churn (dir=both → direction-flip closes), NOT clean strategy trading.

**⛔⛔ BIGGER STOP — the comparison BASELINE itself is contaminated.** The pre-reg compares per-pair against the corpus-v2 (G0) baseline. But G0's ENTIRE class path (all 47 specs, all timeframes) carries the missing-exits defect (`34fabc6d`) — every G0 number is a buy-and-hold / no-proper-exit artifact, NOT the strategy's real behavior. There is **NO clean valid-baseline surface**; a direction read would compare G4's real trading against a G0 artifact = meaningless. This is the 'bigger stop' the STOP protocol reserved for exactly this — surfaced to the operator immediately, everything holds. **The night's discipline (branch-2 STOP → confession run → two-path refutation → execution_path check) caught a contaminated comparison basis BEFORE any direction number could be read over it.**

**OPEN FOR OPERATOR (the 0.5 exam's comparison basis):** the dominant G0↔G4 difference is the missing-exits class-path fix (`34fabc6d`), a defect the frozen pre-reg (Defects 4/7/8/9/10) did not center. Every count change may be classifiable under this ONE named receipted fix (classifiability criterion arguably met — 'the whole corpus flipped because exits landed'), but the equity-DIRECTION criterion is murky (missing-exits can shift equity either way) and 'compared against corpus-v2' presumes corpus-v2 is a valid strategy-behavior baseline, which it is not. Ruling needed on where this leaves item 0.5. NOT resolved at the doer seat.

### §RULING — item 0.5 decomposes into two components (advisor-ruled 2026-07-12, executed)

The (a)/(b) fork was false — both are right, answering the two questions the pre-reg fused. Ruling:
- **Component 1 — ENGINE RE-VALIDATION: MET (strongest form the program has produced).** Every behavioral G0↔G4 difference traces to a NAMED, LANDED, RECEIPTED, RUN-VERIFIED fix: `34fabc6d` (exits injection — the UNIVERSAL count driver: 8 = one held position × 8 windows, head-to-head 1-vs-211 on identical entries) + the OR-fix components (entry-generation, per-pair trigger evidence — the 20 non-1m (e) down-flips STAND, their window-start signature honestly REINTERPRETED as first-entry-per-window OPEN timestamps of the single held position) + the guards + the dormant-common-mode set. **The mesh is EMPTY — zero unexplained (d) remain.** Plus fresh null-cal (0/100 on G4, baseline-INDEPENDENT) + engagement/decision-variance (23/23 overlay engagement). Judge re-validates on COMPLETE CAUSAL ACCOUNTING + demonstrated discriminating power — not on comparing equity to a broken past.
- **Component 2 — DIRECTIONAL-EQUITY vs G0: VOID-FOR-NO-REFERENCE.** Not waived, not failed — UNEVALUABLE. The pre-reg presumed 'the last clean historical snapshot'; the confession proved the precondition FALSE — a no-exits baseline does not MEASURE strategies (it REPLACES the behavior under measurement, vs a Defect-4-style baseline which would BIAS it with known direction). There is no clean historical class-path snapshot; there never was. Criterion recorded VOID with the confession as evidence.
- **Consequence 1 — G4 is the REFERENCE BASELINE** (this IS (b), executed for free — the re-run on the fixed engine already happened: this battery, snapshot-stamped, epoch-accounted). First clean class-path baseline; every future re-baseline compares against it.
- **Consequence 2 — corpus-v2 'overlay HURTS' SUPERSEDED as ARTIFACT.** G0's Mode A/B measured which first-entry opened each window's buy-and-hold — it never validly measured the overlay. The frozen 'overlay HURTS, keep FROZEN' conclusion is VOID as evidence in BOTH directions.
- **Consequence 3 — the read is UNCONTAMINATED and BIGGER.** The within-G4 Mode A/B comparison needs NO G0: 23 informative pairs, real trading, both arms on the corrected engine — never touched by any of this. It is no longer 'does G4 confirm G0's direction' — it is **the FIRST valid measurement of the overlay's value ever taken.** Scope line (pre-written): *within-G4, corrected engine, N=23 informative pairs, no valid prior measurement exists.*

**FULL STOP LIFTS** by its own pre-committed condition (mechanism named with run-level evidence). Open duties resolve under the universal classification: the **six 1m specs → (e) via `34fabc6d`** (entries identical; count delta is purely the exits fix — cleaner than the pre-committed no-baseline exclusion); **`b2deddcb`'s +231 duty SATISFIED** (485 window-holds → 716 real trades = the exits fix, same cause as the whole corpus). NOTHING banked unwinds (vault, 20 non-1m (e) corpus-validated, §V H1 finding, amendment all stand).

**REMAINING SEQUENCE (executing):** (1) INDEPENDENT verification of the confession (fresh-eyes re-derive head-to-head + confirm `34fabc6d` fixing-commit + execution_path=class on 47) — IN FLIGHT, gates everything; (2) record this ruling — DONE (§RULING); (3) the READ — within-G4 direction on the 23 informative pairs, ONCE, under the new scope line → independent re-verification → certify; (4) post-cert: consumption decision (overlay frozen-status), a separate call with operator.

### §VERIFY — independent confession verification (fresh-context agent, grading-integrity doer≠grader, 2026-07-12)

Fresh-eyes agent (no tally/outcome exposure) re-derived the mechanism from code + artifact. **VERDICT: the confession's naming is SOUND. 'Nothing was fabricated.'**
- **Mechanism CONFIRMED** — `git show 48d3cc06:src/engine/backtester.py`: `run_class_backtest` (line 6331) body 6331–7050 has **0 hits** for `_apply_dsl_stop_loss_and_time_stop` / `_apply_dll_halt_to_entries` (defined 3102, called only at 4742 inside `run_backtest`, never the class path). `from_signals` was fed `df['exit_long']`/`df['exit_short']`, which `spec_condition_compiler.py:722` sets **universally all-False** ('framework-owned — NEVER set here'). All-False exits + `n_splits=8` → tc=8. **44/47** clean specs at exactly tc=8 across 1m/15m/4h & MES/MNQ = structural 1-trade/window.
- **Fix commit CONFIRMED** — `34fabc6d` real landed ancestor (Jul 6 03:12), message names **Defect 5** ('run_class_backtest never called `_apply_dsl_stop_loss_and_time_stop()` … Fixed by mirroring run_backtest's exact call pattern'). Current engine injects at 7092, feeds guarded exits to `from_signals` at 7174.
- **execution_path CONFIRMED (exact)** — 47/47 clean specs (all 78 results) `='class'`; report `generated_at` 2026-07-05T02:46Z = unambiguously pre-fix (fix landed 07-06). Named tc verified: `b2deddcb`=485, `331fe15a`=93, `5e5803d1`=82, `ca5564d8`=8.
- **Precision refinements the verifier banked (both already pre-registered in this block):** the 3 high-tc G0 specs (82/93/485) are `direction:both` churning via long↔short *entry reversals* under all-False exits — a different count driver on the SAME missing-exits path, not real exits (§C-notes already state this); and `_apply_trade_management` re-prices the single held trade's exit so it is not literally held-to-window-end in P&L, but never ADDS a trade — count stays 1/window.

**Two gaps the verifier flagged — recorded, and adjudicated as NON-GATING for the within-G4 read (reasoning on the record, relayed for veto):**
1. **G4≈211/window is doer-attested** (recorded only in this analyst's own run doc, not independently re-run — the verifier declined the ~71-min re-run per the reasonable-effort allowance). **NOT load-bearing:** it was illustrative of 'G4 ≫ G0's degenerate 8'; the read never consumes it. The code direction (exits now injected) guarantees a large increase regardless.
2. **The OR-fix entry-generation chain (non-1m directional) was not re-verified in this pass.** The verifier correctly scopes this as gating *'every G0↔G4 difference is a named receipted fix'* — a claim **Component 2 (VOID-for-no-reference) abandons.** Decisively: the within-G4 read compares Mode A vs Mode B on the **same G4 engine, same entries** (arms differ ONLY in `overlay_disabled`); whatever the entry engine does, BOTH arms see it identically and the read measures the overlay delta on top. **Gap #2 cannot touch this read by construction** — it is an axis the read does not stand on. (If a future G0↔G4 attribution claim is made, that chain earns the same rigor first.)

**FULL STOP lift condition (mechanism named with run-level evidence + INDEPENDENT verification) is now fully MET.** Re-entry condition (§WS) satisfied: mechanism named + per-pair classification updated under existing standards + independent verification SOUND.

### §NREAD — effective-N for the read fixed at 25 (operator-confirmed 2026-07-12, mechanical)

N follows the frozen criterion (G4-traded, both-arms-clean), not a running count. The advisor's earlier 'N=23' was a stale pre-straggler tally; §C8 FINAL already read '25 traded'. Confirmed **N = 25** with the standing §C7 engagement criterion applied identically to the two appended straggler rows:
- `ca5564d8` (MNQ/1m): Mode A tc=**1214** → Mode B tc=**192** (overlay engaged A≠B ✓, flags A=off/B=on ✓)
- `589ec512` (MES/1m): Mode A tc=**1538** → Mode B tc=**182** (overlay engaged A≠B ✓, flags A=off/B=on ✓)
Both pass → **all 25 engage; N = 25.** Excluding the four 1m gain-flips would be *selection-on-investigation* (holding them out because they were scrutinized — a bias, not a caution); their exclusions live entirely on the flip/G0 axis ((e) via `34fabc6d`), which the within-G4 read never touches. Solo-clean, epoch-accounted, same engine → IN.

**Two scope-line additions (so the number cannot be misread later):**
- The read carries its **timeframe composition: 15m:6, 1h:2, 4h:1, 5m:12, 1m:4** — and **crude (MCL) absent entirely** (all 26 MCL specs INDETERMINATE at the data gate; no crude in the informative set).
- **Any per-timeframe slicing of the direction is ANNOTATION-TIER ONLY.** The verdict is the pre-registered aggregate read across all 25; no 4am subgroup story — favorable or unfavorable — carries verdict weight.

### §READ — the within-G4 overlay direction, READ ONCE (2026-07-12)

**Scope line (Law 7):** within-G4, corrected engine (`34fabc6d`), N=25 informative pairs (15m:6, 1h:2, 4h:1, 5m:12, 1m:4; crude absent), both arms `TF_ALLOW_FIXED_1` / MAX_CONCURRENT per §epoch, no valid prior measurement exists. Every metric reported; none pre-selected. Usable axes: net P&L (Σdaily_pnls), Sharpe, DSR, DSR-pass (wfe/pbo/b14_ci None corpus-wide, no discriminating power).

**DIRECTION — Mode B (overlay ON) vs Mode A (overlay OFF), all 25:**

| Axis | B>A (overlay helps) | B<A | equal | Aggregate |
|---|---|---|---|---|
| **Net P&L** | **25** | 0 | 0 | portfolio Σ **−$5,694,309 → −$1,133,281** (loss cut ~80%) |
| **Sharpe** | **25** | 0 | 0 | mean **−6.74 → −2.15** |
| **DSR** | 2 | 0 | 23 | mean 0.0 → 0.06 — but **1 of the 2 is a no-trade artifact** (`5e5803d1` B_tc=0 → DSR=0.5 degenerate); only `7ea22e05` (0→1.0) is real |
| **DSR-pass** | +1 | — | — | Mode A **0/25** → Mode B **1/25** (`7ea22e05` MNQ/1h, on 20 trades) |

**CLASSIFIABILITY: PASS — no FULL STOP.** The direction is MONOTONE on P&L/Sharpe/DSR — zero pairs where the overlay worsens any edge axis (trade_count always cuts; that is the mechanism, not an edge axis). Every pair classifies identically. No anomaly, no unexplained/contradictory pair. Directional + classifiable criterion MET.

**INDEPENDENT RE-VERIFICATION (fresh-context grader, doer≠grader, 2026-07-12): NUMBERS CERTIFIED CLEAN.** Grader re-derived all 25 pairs + every aggregate from raw `mode_ab_G4_manifest.jsonl` BLIND: N=25 AGREE, 25/25 P&L AGREE, 25/25 Sharpe AGREE, ΣA=−5,694,308.96→ΣB=−1,133,280.72 AGREE, mean Sharpe −6.7431→−2.1455 AGREE, 1/25 DSR-pass AGREE. **Zero arithmetic discrepancy.** The grader then REFUTED/refined three narrative over-claims (corrected below) and CONFIRMED the near-mechanical read with a correlation the doer had not run.

**VERDICT (scoped, Law 7 — corrected per grader):**
- **The overlay is LOSS-REDUCING on this corpus** — monotone across 25/25 on P&L and Sharpe. The FIRST valid measurement of the overlay's directional value.
- **It OVERTURNS the corpus-v2 'overlay HURTS, keep FROZEN' conclusion** (Consequence 2, §RULING) — that finding was the missing-exits G0 artifact (G0's Mode A/B only measured which first-entry opened each window's buy-and-hold). The clean measurement shows the overlay does NOT hurt; it helps.
- **NEAR-MECHANICAL — now with direct evidence (grader's correlation):** Pearson(dollars-of-loss-avoided [A_pnl−B_pnl], trade-count-reduction [A_tc−B_tc]) = **+0.83** across the 25. Loss avoided tracks trade-count cut tightly → the overlay 'helps' mainly by **trading ~80% less on a negative edge** (mean cut 80.2%, median 78.6% — corrected from the doer's overstated '~85%'), NOT by adding alpha.
- **CRITICAL SCOPE CAVEAT (travels with every citation, corrected):** all 25 are negative-edge in **Mode A** (every P&L a loss, every Sharpe negative). In **Mode B the corpus is NOT uniformly negative** — the grader refuted the doer's 'both arms' phrasing: `7ea22e05` (MNQ/1h) flips to **+$2,908 / +0.39 Sharpe / DSR-pass on 20 trades**, and `5e5803d1` (MES/4h) goes to **exactly $0** (overlay disabled all trades). So the overlay produces ONE thin marginal positive out of 25 — a 20-trade signal that does NOT establish edge-accretion. The read establishes the overlay is loss-reducing HERE; it does **NOT** establish it is edge-ACCRETIVE on PROFITABLE strategies (cutting trades helps a loser, hurts a winner). That needs a positive-edge corpus (H1 fidelity track) — OUT OF SCOPE.
- **EFFECTIVE-N caveat (grader-caught, doer missed):** the 25 pairs contain **2 byte-identical duplicate result-sets** — `6a03c714`≡`a71fb739` and `3f63d906`≡`3c6d439a` (same tc / P&L / Sharpe across differently-named specs; a corpus-duplication tell for the extraction track). So the 25/25 monotone sweep rests on **~23 distinct outcomes**; the direction is unchanged (23/23 distinct), but the count is not 25 independent strategies.

**What this read does and does not license:** it removes the 'overlay HURTS' evidentiary basis for keeping the overlay frozen (that basis was an artifact). It does NOT by itself license flipping the overlay default-ON — that is the **consumption decision**, a separate operator call, because 'loss-reducing on negative-edge specs' (near-mechanical, +0.83) is not 'profit-preserving on positive-edge specs'. The overlay's value on real (positive-edge) strategies is UNMEASURED and stays so until a fidelity-passing corpus exists. Raw read artifact: `docs/replay-results/mode-ab-G4-scripts/within_g4_overlay_read.txt`.

**STATUS: READ CERTIFIED** (numbers independently reproduced clean; narrative corrected per grader; direction + classifiability MET; no FULL STOP). Terminal exam result recorded. See §ANNOT (per-trade decomposition) and §CONSUMPTION (ratified ruling + pre-registered bake-off).

### §ANNOT — per-trade expectancy decomposition (ANNOTATION-TIER; does NOT touch the certified §READ)

Ordered by the operator as the exam's final annotation: one arithmetic pass over the certified manifest to separate **selection skill** (overlay keeps better trades → per-trade loss shrinks) from **pure throttle** (overlay just trades less → per-trade loss unchanged). Metric: net P&L ÷ trade_count, Mode A vs Mode B, per pair. Artifact: `docs/replay-results/mode-ab-G4-scripts/per_trade_expectancy_annot.txt`.

| Measure | Result |
|---|---|
| Portfolio $/trade (24 valid pairs) | Mode A **−$298.64** → Mode B **−$297.45** = **+$1.19/trade (≈FLAT, 0.4%)** |
| Per-pair selection | **9/24 SELECTS** (B less loss/trade) · **15/24 WORSE per trade** · 1 excluded (`5e5803d1` B_tc=0) |
| Median per-trade improvement | **−$22.64** (NEGATIVE — the median strategy's kept trades are worse per trade) |
| Only real selector | `7ea22e05` (+$225/trade — the same 20-trade spec that flipped net-positive) |

**FINDING: the measured overlay (the A+ eligibility gate) is a near-pure THROTTLE, not a selector, on this corpus.** Portfolio per-trade expectancy is flat (+$1.19); on the majority of strategies the surviving trades are slightly WORSE per trade. This confirms the §READ near-mechanical result (+0.83) from an independent angle: the ~80% loss reduction is trade-count throttling, not trade selection. **Honest limit (why this is annotation, not verdict):** on a corpus where every strategy is net-negative there is nothing to select FOR — even a perfect selector would present as a throttle. This proves throttle-behavior HERE; it CANNOT distinguish 'poor selector' from 'good selector with no positive trades to keep'. Selection skill is UNTESTABLE on losers and must be measured post-H1 on positive-edge strategies. It sharpens the bake-off's prior (the incumbent overlay must beat 'does nothing but trade less') without prejudging the race.

### §CONSUMPTION — overlay frozen-status ruling (operator-RATIFIED 2026-07-12)

**RULING: the overlay stays OPT-IN, default OFF.** The agent recommendation is ratified — but the reason on the record is REFRAMED and that reframing is load-bearing:
- **The overlay is NO LONGER frozen because it 'HURTS'** — that verdict is DEAD; it was the broken (missing-exits) engine talking (§RULING Consequence 2). Certified-false.
- **New status: 'UNPROVEN ON WINNERS.'** The exam proved the overlay reduces losses on LOSING strategies near-mechanically (+0.83 correlation with simply trading less; §ANNOT confirms per-trade expectancy is flat). That is real but is NOT the job the overlay is hired for (selection). Flipping it default-ON now would justify a live-config change entirely on evidence from money-losing strategies — the exact scope-stretch the campaign laws catch.

**PRE-REGISTERED (frozen now): the post-H1 OVERLAY BAKE-OFF.** The moment a positive-edge corpus exists (H1 fidelity survivors), the overlay is re-measured for EDGE-ACCRETION on the harness THIS exam built:
- **Candidates:** (1) the A+ eligibility gate (tonight's incumbent — kill-zone / liquidity sweep / 3R / simple confluence count); (2) the **11-factor weighted confluence engine (Path C, Wave 25)** — market structure, liquidity targets, VWAP discount/premium, SMT divergence, killzones, market internals, macro blackout, signal time-decay; (3) ablated variants (add/remove one factor; each must earn its keep).
- **Metric LOCKED NOW (pre-registration, un-gameable):** **per-trade expectancy improvement on positive-edge strategies — NEVER loss reduction** (any throttle fakes loss reduction on losers; §ANNOT is the proof). A candidate wins only by raising $/trade on strategies that already make money.
- **Harness:** within-G4 A/B (this exam's) + the gate-block analyzer (replays every blocked trade, verdicts each filter COSTING vs SAVING) + the factor-ablation gate. Same read-once discipline, same independent doer≠grader verification.
- **Anti-pattern guarded:** on a loser corpus the bake-off 'winner' would be whoever suppresses hardest (death-by-a-thousand-filters). The race is STRUCTURALLY invalid until H1 supplies strategies worth filtering — deferred by design, not omission.

**Queue (post-0.5):** (b) demotion-productionization cert track + **Phase-1 H1 extractor build** run first; the overlay bake-off and the intake-provenance packet queue BEHIND H1 where they belong. **Phase 0 is CLOSED** — judge certified, baseline honest, first true overlay measurement vaulted.

## §V — 331fe15a verification spec (mechanical; hypothesis, NOT concluded)

**Runs TWICE, per-pair** — 331fe15a(→0) AND its byte-identical sibling 5e5803d1(→2); each answer lands on its own (§C2c). (1) Parse the compiled structure; show the long-cross + short-cross conflation in ONE AND-group (`and_group0`: `[5-SMA-cross-above-50 → long]` AND `[5-SMA-cross-below-50 → short]`). **Type-specimen REVISED (pre-number, from the 0-vs-2 evidence): rare-condition-satisfiable, NOT strictly unsatisfiable** — identical structure firing 0 on MNQ/4h and 2 on MES/4h proves the conflated group is data-dependently satisfiable, not impossible. (2) Show G0's fires were bug-dependent (forced-bullish path or counter artifact — whichever the evidence says); account for each sibling's residual G4 fires (0 and 2) on its own data.
- **If verified:** classifies as a **fidelity artifact** — G4's zero is the honest reading of a mis-extracted spec — and becomes a named **H1 finding**: direction-conflated AND-groups are an extraction-defect class the Phase-1 fidelity instrument must catch at compile time (slots into the gradient work; a spec demanding both directions simultaneously is a compile-time absurdity a certificate should catch before an engine sees it).
- **If not verified:** it is an unexplained flip and the FULL STOP owns it.
The hypothesis does not become true by being convenient.

## §A — Activity-table boundary (steer call a, confirmed)

The pre-staged per-spec activity table is validity-tier ONLY while it carries **traded/not-traded per spec per generation + economic grounding** (nonzero-P&L days, fills) and **NO Mode-A-vs-Mode-B numbers** — activity is instrument evidence, overlay deltas are verdict. §C1 conforms (G0-side activity + trigger fields; no overlay-delta columns). §C7's "23/23" is a binary engagement count (Law 1), not overlay magnitudes. Line held.

---

## §C6 — Provenance (Law 2: generation × path × engaged-config)

**Engine (G4):** `backtester.py` — 8501 lines · sha256 `79570c4f8b088b90..` · mtime `2026-07-09T17:53:08` · git HEAD `00c26184`. Byte-stable since **before** the battery started (07-10 00:52) → no engine change mid-run.

**Engaged config:** `TF_ALLOW_FIXED_1=true` (inline at launch, Story-a parity) · `MAX_CONCURRENT_BACKTESTS` = 2 (phase-1 non-1m) / 1 (phase-2 1m solo) · overlay per-arm (A: `overlay_disabled=True`; B: `=False`) verified 50/50. **Four dormant default-ON features all OFF/absent common-mode across BOTH arms** (not a confound): `TF_OR_BRANCHES_ENABLED` (absent→OFF; so the OR-fix's *unconditional* direction-correctness half is what is live, not the flag-gated honoring), `VIX_TIERED_ATR_ENABLED` (absent), event_calendar (unfed), partial-fill (`request.fill_model` unpopulated).

**Dataset stamp** (battery stamped no `dataset_hash` — the Law-2 standing-equipment gap G0 also had; reconstructed here from filesystem content-hash). Symbol map: parquet `CL→MCL`, `ES→MES`, `NQ→MNQ`.

| symbol | tf | sha256(16) | bytes |
|---|---|---|---|
| CL | 1min | `8e357bbd2d956212..` | 22,236,104 |
| CL | 5min | `609808463579d768..` | 6,445,136 |
| CL | 15min | `fa131d808f8ad4b7..` | 2,998,714 |
| CL | 30min | `909e9ca0e2fd0b7a..` | 1,860,289 |
| CL | 1hour | `584e611f0f5c9565..` | 1,125,351 |
| CL | 4hour | `756c93ca4054672b..` | 357,527 |
| ES | 1min | `21a2931ba433e823..` | 24,286,815 |
| ES | 5min | `60aff2474adcd9c5..` | 6,528,042 |
| ES | 15min | `0bf3b820b8ef8b47..` | 2,930,671 |
| ES | 1hour | `607bd99deccf5e9c..` | 1,050,440 |
| NQ | 1min | `a1c194476a88eba6..` | 32,032,102 |
| NQ | 5min | `518dd4a974636a4f..` | 9,304,434 |
| NQ | 15min | `4069442e8daace5f..` | 4,079,996 |
| NQ | 1hour | `cad3eb7f646be28a..` | 1,301,878 |
| NQ | 4hour | `5a3293c710ab0d09..` | 377,787 |

---

## §C5 — Epoch / launch table (common-engine principle)

Battery spanned **2026-07-10 00:52 → 07-11 10:15 UTC (~34h wall)** across operational epochs (OOM deaths ×2, tower restart, split-by-weight reorg, phase-2 solo launch). **All epochs ran identical engine bytes** (mtime 07-09 17:53) **and identical engaged config** → epochs are OOM/restart/scheduling boundaries, **NOT engine or validity boundaries**. Common-engine principle holds; no engine-byte or env-semantics change occurred inside any stage.

| epoch | what | concurrency | note |
|---|---|---|---|
| 1 | phase-1 non-1m initial (15m/1h) | 2 | OOM death #1 (gap 04-07 UTC) |
| 2 | relaunch → tower restart + split-by-weight reorg | 2 | OOM death #2; partition by tf-weight |
| 3 | phase-1 non-1m steady (4h/5m) | 2 | 07-10 18 → 07-11 10:15, resumed PID 634 |
| 4 | phase-1 report written | — | 76→ report: 54 INDETERMINATE / 22 HELPS EDGE |
| 5 | phase-2 1m SOLO (stragglers) | 1 | PID 1608 launch → worker PID 4572, in flight |

*Watcher false-positive caught + logged (Law: verify the alarm instrument): the v1 4h-cap watcher wrote a DONE marker at 1m=7/9; recognized as a timeout artifact, not completion; deleted; replaced with v2 (writes DONE only on true 9/9, separate TIMEOUT flag).*

---

## §C4 — MCL data-gate exclusions (26, expected-abort with exact signature)

All 26 MCL (crude) specs INDETERMINATE on the data-quality gate (fires BEFORE strategy logic). Bad-bar counts are **monotonic with timeframe granularity** — the signature of one fixed ~19h calendar window sampled at increasing frequency, i.e. the **April-2020 WTI negative-settle event** (2020-04-20 14:00 → 04-21 09:15 ET), NOT corruption:

| tf | specs | zero/neg bars |
|---|---|---|
| 4h | 3 | 6 |
| 1h | 2 | 14 |
| 30m | 1 | 25 |
| 15m | 5 | 46 |
| 5m | 12 | 124 |
| 1m | 3 | 358 |

G0-parity (July baseline had identical MCL aborts) → not an A/B confound, but scopes **crude entirely OUT** of the effective read set. **NEVER clamp/floor/interpolate** — the negative prices are real.

---

## §C7 — Engagement + decision-variance (Law 1: overlay is not vacuously dead)

Overlay-flag correctness: **50/50** clean specs (Mode A `overlay_disabled=True`, Mode B `=False`). Decision-variance: **among all 25 TRADED specs, 25/25 show the overlay CHANGING the trade count** (Mode A ≠ Mode B). The overlay demonstrably engages on 100% of specs that trade — engagement evidence satisfied, not a vacuous-parity pass. The 26 `A==B` specs are all zero-trade (0==0, overlay has nothing to act on). One spec (`c029e162`, 1m) has a Mode-B error.

---

## §C1–C3 — Flip analysis (instrument-corrected). CLASSIFICATION HELD for advisor/evening read.

**Record discipline on the prior "7" (full sequence in §R).** The earlier "7 pairs, OR-fix, terminal" classification was ratified on the then-current instrument; it is NOT overwritten. Sequence: 7-classified → this block exposed the degenerate G0 counter → population revised to ~12 candidates → re-classification at the read. Mechanism (OR-fix direction components) STANDS as verified cause; population REOPENED. Every candidate earns its label per-pair, or is unexplained → FULL STOP (§R).

**§C3 — the G0 baseline `trade_count` is a DEGENERATE instrument.** Verified: **44 of 78** G0 clean specs report `mode_a` trade_count = **exactly 8** (= the WF window count; the "1-trade-per-window" broken-counter signature), ALL with `mode_a==mode_b==8` (overlay-invariant). Corroboration: `aaaad0f7` reports G0=8 vs **G4=609** (76× disagreement, identical input); `c029e162` reports 8 trades but **$1.6** total abs P&L (8 futures trades cannot net $1.6). G4 side: **0** specs at exactly-8, healthy spread (0:27, 848:2, 743:2, 609, 595…). → Cross-checking G4 zeros against G0 `trade_count` compares against a broken counter; the flip test is re-run on **actual activity (daily_pnls)**.

**§C2 — zero partition (27 G4 zero-trade specs):** 23 G0-`trade_count`>0→G4-zero "down-flips" + 4 G0-also-errored (no baseline) + 0 stable-zero. **0 upward anomalies** (G0-zero→G4-nonzero = 0 — no spec anomalously *started* trading in G4; 0.5 pre-reg ANOMALY criterion 1 is clean on the upside).

**§C1 — the 23 down-flips, instrument-corrected by G0 daily_pnls activity** (threshold ≥5 non-zero days & >$100 abs → 12 candidate-real / 11 negligible; threshold-dependent, reported as a bound). ALL 23 carry OR-branch/WAIT_BIAS/confirmation triggers (the OR-fix's blast-radius mechanisms):

| spec | sym/tf | name | G0 tc | G0 nz-days | G0 absP&L | wb | or | cf | class |
|---|---|---|---|---|---|---|---|---|---|
| `331fe15a` | MNQ/4h | long_opportunities_mnq_4h | 93 | 109 | 18117.9 | · | Y | Y | cand-REAL |
| `543874f8` | MES/4h | trade_era_scale_in_mes_4h | 8 | 10 | 2800.3 | Y | Y | Y | cand-REAL |
| `534ca8ea` | MES/4h | buy_trades_in_counter_tren | 8 | 10 | 2800.3 | Y | Y | Y | cand-REAL |
| `d3d2d55f` | MNQ/4h | trade_era_scale_in_mnq_4h | 8 | 9 | 885.3 | Y | Y | Y | cand-REAL |
| `0f3a1418` | MNQ/4h | buy_trades_in_counter_tren | 8 | 9 | 885.3 | Y | Y | Y | cand-REAL |
| `4be0db1d` | MES/15m | bos_and_fvg_or_fvg_mes_15m | 8 | 6 | 8553.6 | Y | Y | · | cand-REAL |
| `a18406a7` | MES/15m | long_entry_mes_15m | 8 | 6 | 8553.6 | · | Y | Y | cand-REAL |
| `fb2e92ab` | MES/1h | jump_in_downtrend_mes_1h | 8 | 6 | 3277.4 | Y | Y | Y | cand-REAL |
| `9b871b65` | MNQ/30m | discount_price_to_buy_from | 8 | 6 | 2396.0 | · | Y | Y | cand-REAL |
| `913f40c4` | MNQ/15m | bos_and_fvg_or_fvg_mnq_15m | 8 | 5 | 5902.9 | Y | Y | · | cand-REAL |
| `93534762` | MNQ/15m | long_entry_mnq_15m | 8 | 5 | 5902.9 | · | Y | Y | cand-REAL |
| `0dfd11ee` | MNQ/1h | jump_in_downtrend_mnq_1h | 8 | 5 | 2700.2 | Y | Y | Y | cand-REAL |
| `979ba21b` | MES/30m | discount_price_to_buy_from | 8 | 4 | 967.5 | · | Y | Y | negligible |
| `9955f721` | MNQ/5m | opening_range_breakout_orb | 8 | 2 | 333.0 | Y | Y | Y | negligible |
| `84928232` | MNQ/5m | new_high_acceptance_mnq_5m | 8 | 2 | 333.0 | Y | Y | Y | negligible |
| `ff718865` | MNQ/5m | breakout_capture_mnq_5m | 8 | 2 | 333.0 | Y | · | Y | negligible |
| `c1217b13` | MNQ/5m | hammer_candle_long_side_mn | 8 | 2 | 333.0 | Y | Y | Y | negligible |
| `fc4bf082` | MES/5m | opening_range_breakout_orb | 8 | 2 | 98.5 | Y | Y | Y | negligible |
| `85776068` | MES/5m | breakout_capture_mes_5m | 8 | 2 | 98.5 | Y | · | Y | negligible |
| `f6ff3004` | MES/5m | new_high_acceptance_mes_5m | 8 | 2 | 98.5 | Y | Y | Y | negligible |
| `710a9ed3` | MES/5m | hammer_candle_long_side_me | 8 | 2 | 98.5 | Y | Y | Y | negligible |
| `81002a32` | MES/1m | manipulation_trade_mes_1m | 8 | 1 | 11.2 | Y | Y | Y | negligible |
| `c029e162` | MNQ/1m | manipulation_trade_mnq_1m | 8 | 1 | 1.6 | Y | Y | Y | negligible |

**§C2b — SYMMETRIC gain-flip sweep (G0-clean-dead → G4-traded, ACTIVITY instrument).** The OR-fix predicts the opposite sign; the pre-reg anomaly criterion is symmetric. TRUE gain-flip candidates = G4-traded specs with a CLEAN-but-dead G0 baseline (≤1 non-zero P&L day). Classification held for the sitting under the closed taxonomy + the §R component-(ii) confirm:True prediction (**confirm** column is the pre-registered signature):

| spec | sym/tf | name | dir | G0 tc | G0 nz-days | G0 absP&L | G4 traded | wb | or | confirm |
|---|---|---|---|---|---|---|---|---|---|---|
| `67d67ba0` | MNQ/1m | bullish_candle_formati | long | 8 | 1 | 1.6 | 1230 | · | Y | **Y** |
| `f3babbdd` | MES/1m | bullish_candle_formati | long | 8 | 1 | 11.2 | 1551 | · | Y | **Y** |
| `ca5564d8` | MNQ/1m | avoiding_two_mistakes_ | long | 8 | 1 | 1.6 | 1214 | Y | Y | · |
| `589ec512` | MES/1m | avoiding_two_mistakes_ | long | 8 | 1 | 8.7 | 1538 | Y | Y | · |

TRUE gain-flip candidates: **4** (all 1m LONG). Staged signature read (routing at sitting): **2 carry confirm:True** → component-(ii) prediction holds for those; the remainder carry wait_bias-not-confirm → component-(i) structurally cannot explain a long gain → flagged AMBIGUOUS for per-pair read. The all-long direction mix does NOT fit forced-bullish-suppresses-shorts — the confirm signature is what discriminates, exactly as pre-registered.

**No-G0-baseline (exits the flip taxonomy — a G0-error has no side to flip from; forcing it in blends "changed" with "unmeasured"):**
| spec | sym/tf | name | G0 disposition | G4 |
|---|---|---|---|---|
| `ae7a2560` | MES/15m | long_entry_or_short_en | G0 ValueError: RECONCILIATION FAILED: equ | traded 705 |

Disposition (Law 7 per-read scoping): **EXCLUDED** from the G0→G4 shift read (no baseline to compare); **INCLUDED** in the within-G4 Mode-A-vs-B read IF its G4 run is clean. Cause stated (Defect-4 reconciliation INDETERMINATE). *Straggler spec-2 (`589ec512`, MES mirror of `ca5564d8`) may join the true-gain class on landing → the same confirm:True signature check applies identically, nothing assumed from the sibling.*

### §C2c — G0 baseline-validity precondition (door-1 evidence, pulled pre-number)

Per the ruling, before the 1m specs enter the flip taxonomy their G0 baseline is checked for run-validity. Result: **neither specified tell fires as hoped.**
- **Runtime tell — NEGATIVE.** G0 1m specs each ran **~2.7h** (589ec512 2.78h, ca5564d8 2.73h, bullish_candle pair 2.71h, manipulation pair ~2.70h) — real multi-hour runs, NOT truncated-to-minutes. G4 takes 5–9h (2–3×, explained by G4's added analytics). The truncation hypothesis does not hold.
- **Completeness tell (wfe/pbo/b14_ci) — NO DISCRIMINATING POWER.** wfe=None AND pbo=None for **all 47** G0 clean specs — the 44 exactly-8 AND the 3 genuinely-traded (`b2deddcb` tc=485/410 days, `331fe15a` tc=93/109, `5e5803d1` tc=82/94). A G0-wide reporting gap, not a valid/degenerate separator.
- **The exactly-8 degeneracy is CORPUS-WIDE, not 1m-specific.** All 44 exactly-8 specs (1m + non-1m) share: tc=8 (=WF window count), mode_a==mode_b (overlay-invariant), dsr∈{0.0,1.0}, sharpe∈{±0.5}, wfe/pbo None, 1–10 non-zero P&L days. The ONLY signal separating them from valid-G0 is the **activity instrument itself** (non-zero-days / tc-diversity) — not an independent run-validity tell.

**Door-1 RULING (operator, 2026-07-11 ~15:30 — (a)/(b) was a false dichotomy fusing two questions; separated):**
- **Flip taxonomy → (b):** door-1 reserved for HARD run-failures (ae7a2560's reconciliation error) ONLY. The exactly-8 fingerprint does NOT waive classification — (a) would be a bulk waiver lifting the engine-validation net off 94% of the corpus in one motion; if even one exactly-8 spec flipped for a reason unrelated to the verified mechanism (a 5th dormancy, an unknown fix side-effect), (a) guarantees we never look. Criterion-4 (fail toward suspicion) applied to the fork itself. Every exactly-8 spec earns a per-pair label; **neither-trigger-yet-flipped → FULL STOP** (the mesh (a) would have cut).
- **Cross-generation claims → (a) as SCOPE MARKER, not exclusion:** the degenerate-G0 fingerprint (tc=8 broken counter, quantized dsr∈{0,1}/sharpe∈{±0.5}, absent wfe/pbo) disqualifies those 44 pairs from before/after MAGNITUDE or overlay-agreement claims. For them the only cross-gen sentence allowed is the classification itself. Richer before/after reads live ONLY on valid-baseline pairs.

**Fingerprint refinement — arm-identity re-check (done, result-independent):** the `mode_a==mode_b` component was the BROKEN COUNTER, not behavioral identity. Re-judged on the activity instrument (P&L-day patterns, not counts): **44/44 exactly-8 specs have mode_a activity ≠ mode_b activity.** Zero truly overlay-uninformative. So 'overlay-invariant' DROPS from the fingerprint; G0's overlay carried real information the counter hid (fail-toward-more-information). The degenerate call now rests on tc=8 + quantized-stats + absent-analytics alone.

**Valid-baseline comparison surface (the 3 non-exactly-8 G0 specs — the only pairs that may carry richer cross-gen claims):**
| spec | sym/tf | strategy | dir | G0 (tc/nz-days) | G4 (tc/nz-days) | role |
|---|---|---|---|---|---|---|
| `b2deddcb` | MNQ/15m | long_entry_or_short_entry | both | 485 / 410 | 716 / 701 | **clean both-traded before/after pair** |
| `331fe15a` | MNQ/4h | long_opportunities | both | 93 / 109 | 0 / 0 | full flip; §V candidate |
| `5e5803d1` | MES/4h | long_opportunities | both | 82 / 94 | 2 / 2 | near-full flip; §V SIBLING of 331fe15a |

`331fe15a` is the exam's most load-bearing single spec: 1-of-3 valid baseline × real down-flip × §V type-specimen. Two pre-registrations on the valid surface (both NOW, pre-number — the surface is precious so its members get MORE scrutiny, not an exemption):

**PRE-REG (b2deddcb count-classification duty).** Its **485→716 (+231, +48%)** count change must classify BEFORE the directional read anchors on it — count-stability is not flips-only; an unexplained count change is **ANOMALY criterion 2 (unmodeled interaction) → FULL STOP**. Candidates (label at sitting, trade-level evidence): Defect-10 (b)-class occupancy-freed (removed look-ahead trades free slots — receipt documented "freed bigger losers"); OR-fix component-(ii) gain (**config carries confirm=True → candidate viable**); or (c) timing-shifted. One with evidence → it anchors the directional read with its delta explained; none → the pre-reg's own stop applies to the exam's best pair.

**PRE-REG (§V runs TWICE, per-pair — NOT inheritance).** `331fe15a`(→0) and `5e5803d1`(→2) have **BYTE-IDENTICAL compiled structure** (verified: same 6 and_groups, same conflated `and_group0` = 5-SMA-cross-above-50+long AND 5-SMA-cross-below-50+short). So the 0-vs-2 split is NOT structural — and it **falsifies strict-unsatisfiability** (an unsatisfiable group is exactly 0 in BOTH). Corrected §V type-specimen: **rare-condition-satisfiable bidirectional-confirmation conflation** (the rare long+short co-confirmation never aligns on MNQ/4h → 0, aligns twice on MES/4h → 2; data-driven, same fidelity defect). §V is run once per spec; 5e5803d1's 2 residual trades get their own account under the same method. "§V decides 2/3 of the surface" is the HYPOTHESIS the two runs confirm or split. *(This pre-reg's prior form — advisor-predicted "residual trades ⇒ structure differs" — was FALSIFIED by the byte-identical evidence; logged, corrected, sharper. Two-path rule's 3rd catch this week, this one falsifying the advisor seat.)*

**TWO-AXIS separation (each cratering sibling carries TWO independent per-pair questions; a spec can be OR-fix on one AND artifact on the other — the record holds both, no blend):**
1. **MECHANISM (flip axis):** a rare-satisfiable structure fired 93/82× in G0 — something made the rare group fire regularly (a bug bypassing part of the AND). Trigger fields present → OR-fix class; absent → the G0 firing is itself unexplained → the FULL-STOP mesh catches it.
2. **FIDELITY (§V axis):** is the long-cross-AND-short-cross conflation what the transcript actually demands, or an extraction artifact? Answered from compiled conditions vs the source, INDEPENDENT of why G0 fired.
Both land per-pair at the sitting; neither answer substitutes for the other.

**The one clean anomaly — `331fe15a` (MNQ/4h `long_opportunities`, G0=93 trades/109 days/$18.1k → G4 zero).** Not an exactly-8 artifact. CANDIDATE cause (hypothesis, verify at read): direction=BOTH; `and_group0` conflates `[5-SMA-cross-above-50→long]` AND `[5-SMA-cross-below-50→short]` into ONE **AND** group → **unsatisfiable** (5-SMA cannot be simultaneously above and below 50-SMA). G4's corrected confirmation-direction logic finds it unsatisfiable → 0 (arguably *correct*); G0's confirmation-direction-*discard* bug fired it anyway → 93. **This points at extraction fidelity (a bidirectional strategy compiled to an unsatisfiable AND group), not an engine-validity defect.**

**The interpretive fork (JUDGMENT — held for advisor/evening read, not resolved here):**
- **Reading A ("expected fix-working"):** G0's spurious signals came from the pre-fix direction bugs; G4 correctly zeroes them; flips are *expected*, not anomalies.
- **Reading B (strict 0.5 pre-reg):** every zero↔nonzero flip is an ANOMALY (criterion 1) needing per-spec adjudication before the read; ~12 candidate-real is a non-trivial population touching effective-N.

---

## §C8 — Effective-N (Law 7) — FINAL @ 78/78

| stage | N | cause |
|---|---|---|
| launched (strategy_list) | 78 | — |
| − stragglers pending (1m) | −0 | ca5564d8, 589ec512 (in flight; §0 branch if solo-death) |
| = done | 78 | — |
| − MCL data-gate | −26 | April-2020 negative-settle (crude untested) |
| − OOM / other errors | −0 | **0** (all 26 errors are MCL) |
| = clean-ran (effective-N) | **52** | MES/MNQ only |
| — of which TRADED (informative) | 25 | overlay engaged 23/23 |
| — of which zero-trade | 27 | 23 down-flips (classification held) + 4 G0-err |

**Effective-N (current) = 50 clean-ran** (MES/MNQ), traded-informative = 23. The 2 stragglers (both 1m, potentially traded-informative) append to this line when they land; if the §0 resource-infeasible branch fires, effective-N holds at what completed with the cause stated.

---

*Assembled 2026-07-11 while phase-2 1m stragglers complete. No directional number read. Next: append straggler rows → full-78 validity→direction→classifiability read, one evening sitting.*
