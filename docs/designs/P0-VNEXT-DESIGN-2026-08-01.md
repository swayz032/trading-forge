...
>
> ### ✅★★★★★ HOW SEMANTIC INAPPLICABILITY IS CARRIED INSTEAD — **BY VALUE, NEVER BY OMISSION**
> **ALL FIVE PROJECTION KEYS ARE ALWAYS EMITTED BY BOTH LANES.** A semantically inapplicable field is emitted as JSON `null`. **`MISSING` therefore means the LANE FAILED TO EMIT — a defect — and never *"this did not apply."*** ★★★★★ **`SEMANTIC INAPPLICABILITY IS NOT ENCODED BY DELETING A REQUIRED KEY.` A schema in which absence is sometimes legitimate has no way to report the absence that is not.**
> ✅ **THE SKIP PROOF MOVES WHOLLY TO CLAIM `B`, and runs AFTER the sealed claim-`A` projection.** The `9` `NOT-APPLICABLE` cells still emit their named skip witnesses — **that requirement is unchanged and lives in §4** — but they are a CLAIM-`B` output and no claim-`A` verdict is a function of them.
>
...
> ### 🛑★★★★★ THE CAPABILITY AUTHORITY IS **PROCESS/DEPENDENCY ISOLATION** — NOT A DIGEST, NOT A SIGNATURE, NOT AN IMPORT SCAN
> ⚠️ **A behavioural control cannot be the capability authority; it can only sample it.** The authority is structural, and the PREFERRED form is the strongest one available here:
> **PREFERRED — SEPARATE PROCESS.** `project()` runs in its own process which receives **ONLY** the serialized lane output plus the closed plain-data projection contract, and is **DENIED filesystem, network and environment access to the ledger and oracle paths.** ★★★ **Then "did not read the ledger" is a property of the sandbox, not a claim about the code.**
> ⚠️★★★★★ **IF A SAME-PROCESS MODULE IS RETAINED INSTEAD, THE DESIGN OWES THE AMBIENT SURFACE EXPLICITLY — because scanning NAMED IMPORTS is not a boundary:** how **dynamic `import()`** · **generic filesystem reads** · **`globalThis` / module-scope singletons and caches** · **environment variables** · and **TRANSITIVE dependencies** are each denied or **FAIL CLOSED.** ★★ **`[UNRESOLVED — NAMED]` this document does not yet specify same-process ambient denial, which is exactly why the separate-process form is preferred rather than merely suggested.** `AN UNSPECIFIED DENIAL IS AN OPEN CAPABILITY.`
...
⚠️★★★★★ **THIS MATRIX IS `32` MUTATIONS **PLUS** `1` CLEAN CONTROL — `33` NUMBERED ROWS.** `[COUNTED FROM THE TABLE BELOW BY PARSE, after the rows landed; NOT copied from any prior statement of the figure.]`
★★★★★ **STATED IN THAT FORM BECAUSE THIS CAPTION HAS NOW BEEN WRONG OR STALE THREE TIMES: `AR-566`'s *"`23` MUTATIONS"* (truth `22`+`1`), then `24`+`1`, then `29`+`1` — each correct until the next rows landed.** ⚠️ **`A CONTROL COUNTED AS AN ATTACK INFLATES A SUITE'S ADVERSARIAL STRENGTH BY EXACTLY THE ROW THAT PROVES IT IS NOT ALWAYS RED`, and `A COUNT CARRIED ACROSS AN EDIT IS STALE THE MOMENT THE EDIT LANDS.`**
...
| 7 | a `NOT-APPLICABLE` cell, both lanes emitting JSON `null` for it | ★★★★★ **claim `A` UNAFFECTED (the key IS emitted) · claim `B` emits the named skip witness and NO predicate** | §4 claim-`B` skip witness |
...
| **33** | **clean control — unmutated** | **GREEN**, with the frame, resolved scope and digest printed | all |
