...
>
> #### 🛑★★★★★ DATED CORRECTION NOTE — MATRIX ROW `54`, REPLACED 2026-08-02 (`R-546 §5.13`, entered by `AR-592`)
> ⚠️ **PRESERVE-AND-STRIKE, NOT DELETE. The prior row-`54` text is retained here verbatim so the record shows what was claimed and when:**
> > ~~*"**CJS MODULE-WRAPPER `this` INJECTION** — a CommonJS module assigning an injector onto the wrapper `this`; **EVERY enumerated forbidden token ABSENT** | **RED — the MODULE-SYSTEM / wrapper-`this` channel NAMED** … **GREEN NEIGHBOUR: the SAME module as ESM (`typeof this === "undefined"`) stays GREEN**"*~~
>
> 🛑★★★★★ **WHY IT WAS FALSE, MEASURED:** `R-544 §1` established — and `AR-591` confirmed from a second angle — that **the "ESM twin" was the red WITH ONE LINE DELETED**, i.e. the mutation run backwards, and that **the identical `this`-statement text produces IDENTICAL diagnostics in `.mjs` and `.cjs`.** ★★★★★ **So the old row proved *"a module-scope `this` statement is detected in a TS/ESNext parse"* — NOT *"the CommonJS wrapper channel is excluded from the effective artifact."* `A TWIN THAT DIFFERS BY THE DELETED LINE IS NOT A CONTROL.`**
> ✅ **THE REPLACEMENT IS EXECUTION-PROVEN, BOTH ARMS ACTUALLY RUN** (`R-546 §5.13`'s condition): the new row `54` is a byte-identical container twin whose CJS arm reddens on `1b-S:module-system` **alone**, and the emitted artifacts of both arms are **executed** (`"undefined"` vs `"object"`). ⚠️ **The external read's conditional-wrapper variant remains a `[RELAYED]` CANDIDATE (`R-546 §4`) and is NOT adopted here — nothing in this replacement rests on it.**
> ⚠️ **The original channel is retained as row `54(c)` under an honest caption rather than deleted.**
...
| **54** | 🛑★★★★★ **CJS CONTAINER — TRUE TWIN.** **ONE source text, byte-identical, placed in TWO containers** (`twin.cts` vs `twin.mts`; also `twin.cjs` vs `twin.mjs`). The text contains **NO CommonJS token and NO module-scope `this`** — the ONLY difference between the arms is the file extension. | 🛑★★★★★ **RED on the CJS arm — the MODULE-SYSTEM channel NAMED, AND NOTHING ELSE** `[MEASURED, `AR-592`, `prototypes/p0-vnext-admission/run.mjs`]`: `sameBytes=true (131B)` · CJS arm → `1b-S:module-system` **as its ONLY catcher** · ESM arm **ADMITTED**. ✅ **BOTH ARMS EXECUTE:** emitted `probe.mjs` → top-level `typeof this` = `"undefined"`; emitted `probe.cjs` → `"object"` (**the positive control — without it the ESM absence is a dead probe**). ⚠️ **The row FAILS if the arms differ by anything other than the container, or if any text-level catcher fires in either arm.** | §2 `1b-S` **module-system pin (ESM only)**, decided by the **effective-module tuple** (extension + nearest `package.json` `"type"`), cross-checked against `ts.impliedNodeFormat` — ⚠️ **NOT the ambient-reads row and NOT the identifier scan, both SILENT in both arms** |
| **54(c)** | ⚠️ **MODULE-SCOPE `this` STATEMENT (text-level).** Retained from the ORIGINAL row 54, under an honest caption. | ⚠️★★★★★ **NOT a module-system discriminator, and this is MEASURED, not argued** `[MEASURED, `AR-591`]`: the identical text produces **IDENTICAL diagnostics in `.mjs` and `.cjs`**. Under the pinned surface it is `TS2532`+`TS2540` → **`caught_by_typechecker`**, i.e. a real catch by a different layer. | the type-checker — **not** the `1b-S` module-system rule |
