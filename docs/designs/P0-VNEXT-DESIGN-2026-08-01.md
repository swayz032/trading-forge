...
> **PREFERRED FORM: A ZERO-IMPORT LEAF.** Every surface below is a CLOSED SET — anything not listed is FORBIDDEN, and the rule names the offending symbol or path.
> | surface (`1b-S`) | ALLOWED (closed set) | FORBIDDEN — the rule NAMES the offending symbol/path |
> |---|---|---|
> | **imports** | ⚠️ **none** in the preferred form; otherwise an enumerated, frozen allow-list containing no ledger/oracle/filesystem/network module | any unallowlisted import · any transitive edge reaching a forbidden module |
...
> | **direct ambient reads** | ★★★★★ **nothing** | ★★★★★ **`globalThis` · `window` · `process` / `process.env` · any host-global identifier not in the allow-list** ⚠️ **its own row, because it is neither an import nor a captured reference and was previously catchable only as a side clause** |
...
> |---|---|---|
> | **the lane object + the frozen normalization contract** | plain data only: string · number · boolean · `null` · array · plain object, recursively | ★★★★★ **any accessor (`get`/`set`) descriptor · any function-valued field · callbacks · thunks · objects carrying methods · prototype-bearing class instances** |
...
> 4. **TWO CHECKS FAIL THE RUN, AND THEY ARE REPORTED SEPARATELY.** ⚠️★★★★★ **`4a` BUILD-TIME (`1b-S`, parser-based): any forbidden import path, dynamic-load construct, mutable module-scope holder, direct ambient read, or captured reference reaching `project()`'s closure — **names the offending symbol/path.** · `4b` RUNTIME (`1b-R`, descriptor walk at the call boundary): any accessor or function-valued field on the value handed to `project()` — **names the offending key.**
...
> **`4a`:** add a ledger-reader import to the closure · add a filesystem module · introduce a dynamic `import()` · add a transitive dependency that pulls in either · 🛑★★★★★ **the one the import-based four cannot reach: INJECT AN EXPECTATION READER THROUGH A MODULE-LOCAL SETTER OR CALLBACK **WHILE THE IMPORT GRAPH STAYS CLEAN**** · ★★★★★ **direct `globalThis` read** · ★★★★★ **direct environment read (`process.env`)** · ★★★★★ **a mutable cache / lazily-initialised holder WITH NO SETTER EXPORT** ⚠️ **(the setter row does not cover it — this one is written and read entirely inside the module).**
> **`4b`:** ★★★★★ **a FUNCTION-VALUED FIELD on the runtime input** · ★★★★★ **a GETTER/ACCESSOR on the runtime input, and it CARRIES AN INVOCATION COUNTER WHOSE REQUIRED VALUE IS `0`.**
> 🛑★★★★★ **THE ADMITTED INSPECTION MECHANISM FOR `4b` IS `Object.getOwnPropertyDescriptors`, AND IT IS THE **SOLE** ADMITTED ONE `[MEASURED HERE, `AR-578 §2`, node v24.13.0, invocation counter]`. **EXCLUDED BECAUSE EACH WAS MEASURED INVOKING THE GETTER EXACTLY ONCE: spread `{...lane}` · `JSON.stringify` · `Object.values` · `Object.entries` · `structuredClone`.**** ★★★★★ **`THE OBVIOUS WAY TO INSPECT A VALUE IS THE WAY THAT EXECUTES IT` — `5` of `7` idioms, measured, twice, by two independently authored harnesses.**
...
> ★★★★★ **GREEN DISCRIMINATING NEIGHBOURS, SO NO ROW DEGENERATES INTO `reject everything`:** a module-scope DEEPLY-FROZEN plain-data constant stays GREEN (vs the setter/cache rows) · an allow-listed pure helper import stays GREEN (vs the import rows) · a plain-data lane object with the identical KEY SET stays GREEN (vs both `4b` rows). `A CONTROL MUST DISCRIMINATE, NOT MERELY TRIGGER.`
...
⚠️★★★★★ **THIS MATRIX IS `42` MUTATIONS **PLUS** `1` CLEAN CONTROL — `43` NUMBERED ROWS, CONTIGUOUS `1..43`, ZERO DUPLICATES, CONTROL LAST.** `[COUNTED BY A SECTION-ANCHORED PARSE AFTER THE ROWS LANDED; NOT copied from any prior statement of the figure — including this document's own caption two edits ago, which read `40+1` and was already stale when the last two rows landed.]`
★★★★★ **SIXTH RECOMPUTATION. The caption has read `23`, `22+1`, `24+1`, `29+1`, `32+1`, `33+1` and now `42+1`, each correct until the next rows landed.**
🛑★★★★★ **AND THE PARSE IS NOW ANCHORED TO **THIS SECTION**, NEVER TO A ROW SHAPE — BECAUSE A ROW SHAPE HAS FOOLED THIS DESK TWICE ON THE SAME TABLE.** `[MEASURED HERE, WITH A POSITIVE CONTROL]` **anchored to `## 10`: `43` rows. The IDENTICAL row-shape pattern with NO anchor: `48` rows — the `5` extra are the field-mapping table at `:196–200`, whose rows are numbered `1..5` and are not mutations at all.** ★★★★★ **THE CONTROL IS THE POINT: the parser CAN see those five, so the anchor is what EXCLUDES them rather than blindness doing it.** `A CENSUS IS BOUNDED BY ITS SURFACE AS WELL AS ITS PATTERN` · `PARSE A TABLE BY ITS SECTION ANCHOR, NEVER BY ITS ROW SHAPE.`
...
| **34** | 🛑★★★★★ **CLEAN-IMPORT CAPTURED-READER INJECTION** — feed an expectation reader into `project()` through a module-local **setter/callback** while the **IMPORT GRAPH STAYS CLEAN** | 🛑★★★★★ **RED — the rule FAILS and NAMES the injected symbol** · ★★★ **PAIRED NEIGHBOUR: a module-scope IMMUTABLE PLAIN-DATA CONSTANT stays GREEN, proving the rule rejects injected STATE and not every module-scope reference** | §2 `1b-S` closed **export/state** surface (parser) — ⚠️ **NOT the import-graph check, which stays silent here** |
| **35** | ★★★★★ **DIRECT `globalThis` READ** — `project()`'s module reads `globalThis.__ledger`; **import graph CLEAN** | RED — build fails, the `globalThis` reference **NAMED** · ★★★ **GREEN NEIGHBOUR: an allow-listed pure helper import stays GREEN** | §2 `1b-S` **direct-ambient-read** row (parser) — ⚠️ **NOT the import-graph check, which stays silent here** |
...
| **43** | **clean control — unmutated** | **GREEN**, with the frame, resolved scope and digest printed | all |
...
**`NO PROMISE MAY EXIST WITHOUT A MATCHING CATCHER` is checked in BOTH directions, and *"promised AND caught"* means the DIRECT SYNTACTIC scope narrowed in §THE CAPABILITY CONTRACT.** `[MEASURED HERE, section-anchored parse]`
```
PROMISE (forbidden channel, 1b-S / 1b-R)                          -> CATCHER (matrix row)
unallowlisted import / transitive edge to a forbidden module      -> 26
captured reference via a module-local setter/callback export      -> 34
direct globalThis / window read                                   -> 35
direct environment read (process.env)                             -> 36
mutable cache / singleton / lazy holder, no setter export         -> 37
module-scope state that is only SHALLOW-frozen                    -> 38
function-valued field on the runtime input                        -> 39
accessor (get/set) descriptor on the runtime input                -> 40
dynamic loading: import() / require / eval / new Function         -> 41
prototype-borne method or inherited getter on the runtime input   -> 42
  promises with NO catcher ....... EMPTY
  catcher rows with NO promise ... EMPTY   (capability rows derived FROM THE MATRIX: 26,34,35,36,37,38,39,40,41,42)
```
⚠️🛑★★★★★ **AND THE INSTRUMENT WAS AUDITED BEFORE ITS RESULT WAS BELIEVED, BECAUSE ITS FIRST DRAFT WAS A TAUTOLOGY: the reverse direction computed `set(MAP.values()) − set(MAP.values())`, **EMPTY BY CONSTRUCTION** — a check that could never convict, printing the answer I wanted.** ✅ **REPLACED: the capability-family rows are now derived FROM THE MATRIX (rows whose catcher column names `1b-S`/`1b-R`/dependency-boundary) and compared against the map.** ★★★★★ **BOTH DIRECTIONS CARRY A NEGATIVE CONTROL `[MEASURED HERE]` — point a promise at a non-existent row and FORWARD goes red; drop row `42` from the map and REVERSE reports `[42]`. `AN EMPTY DIFFERENCE IS ONLY EVIDENCE IF THE CHECK COULD HAVE BEEN NON-EMPTY.`**
✅★★★ **THE REVERSE CHECK EARNED ITS KEEP IMMEDIATELY: it did NOT see row `34`, because that row's catcher still named the pre-split `requirement 1b` — ambiguous between the two contracts after the split. Retargeted to `1b-S`.** `A WITHDRAWN NAME SURVIVES UNTIL EVERY OPERATIVE CARRIER IS REMOVED`, and a catcher column is an operative carrier.
