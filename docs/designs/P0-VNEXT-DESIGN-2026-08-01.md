...
>
> ### 🛑★★★★★ REQUIREMENT 1b — **THE MODULE'S CLOSED SURFACES. AN IMPORT GRAPH CLOSES IMPORTS; IT DOES NOT CLOSE STATE INJECTION.**
> **PREFERRED FORM: A ZERO-IMPORT LEAF.** Every surface below is a CLOSED SET — anything not listed is FORBIDDEN, and the check names the offending symbol.
> | surface | ALLOWED (closed set) | FORBIDDEN |
> |---|---|---|
> | **imports** | ⚠️ **none** in the preferred form; otherwise an enumerated, frozen allow-list containing no ledger/oracle/filesystem/network module | any unallowlisted import · dynamic `import()` · any transitive edge reaching a forbidden module |
...
> | **module-scope state** | ⚠️ **immutable plain-data constants ONLY** (frozen, no getters) | ★★★★★ **any mutable module-scope binding · caches · singletons · lazily-initialised holders** |
> | **inputs to `project`** | the lane object + the frozen normalization contract, both plain data | ★★★★★ **callbacks · function-valued fields · thunks · objects carrying methods or accessors** |
> | **free / captured references** | only the allow-listed immutable constants above | ★★★★★ **any other free or captured reference, including `globalThis`, environment reads, and closures over injected values** |
...
> ⚠️★★★ **AND `:99`'s BAN ON CALLBACKS IN THE PROJECTION DATA CONTRACT DOES NOT DISCHARGE THIS: that is the ARGUMENT surface, this is the MODULE surface, and a zero-import module can still receive an expectation reader through an exported setter.** `TWO DIFFERENT SURFACES, TWO DIFFERENT CLOSURES.`
...
> **RED-PROOFS IT OWES (each a build-time failure, each naming the offending path):** add a ledger-reader import to the closure · add a filesystem module · introduce a dynamic `import()` · add a transitive dependency that pulls in either · 🛑★★★★★ **AND THE ONE THE IMPORT-BASED FOUR CANNOT REACH: INJECT AN EXPECTATION READER THROUGH A MODULE-LOCAL SETTER OR CALLBACK **WHILE THE IMPORT GRAPH STAYS CLEAN** — the rule must FAIL and NAME the injected symbol.** ★★★★★ **PAIRED WITH ITS DISCRIMINATING NEIGHBOUR: a module-scope IMMUTABLE PLAIN-DATA CONSTANT must stay GREEN, so the rule is `reject injected STATE`, not `reject every module-scope reference`.** `A CONTROL MUST DISCRIMINATE, NOT MERELY TRIGGER.`
> ⚠️ **A channel this contract does NOT claim to deny gets NO red-proof and NO claim — post-review dependency behaviour is named OUT OF SCOPE rather than left to be assumed covered.** ⚠️★★★ **`globalThis` and environment reads are NO LONGER out of scope: requirement `1b` closes the free/captured-reference surface and lists them as FORBIDDEN, so they are now promised AND caught.**
...
⚠️★★★★★ **THIS MATRIX IS `34` MUTATIONS **PLUS** `1` CLEAN CONTROL — `35` NUMBERED ROWS.** `[COUNTED FROM THE TABLE BELOW BY PARSE, after the rows landed; NOT copied from any prior statement of the figure.]`
★★★★★ **FIFTH RECOMPUTATION. The caption has read `23`, `22+1`, `24+1`, `29+1`, `32+1` and `33+1`, each correct until the next rows landed.** ⚠️ **AND THIS ROUND THE RE-PARSE CAUGHT A REAL DEFECT RATHER THAN JUST CONFIRMING A NUMBER: the new row was first written `35` while the old control WAS `34`, leaving the sequence GAPPED at `34`. `A CONTIGUITY CHECK IS NOT DECORATION — IT IS THE ONLY THING THAT SEES A NUMBER YOU CONSUMED.`**
...
| **34** | 🛑★★★★★ **CLEAN-IMPORT CAPTURED-READER INJECTION** — feed an expectation reader into `project()` through a module-local **setter/callback** while the **IMPORT GRAPH STAYS CLEAN** | 🛑★★★★★ **RED — the rule FAILS and NAMES the injected symbol** · ★★★ **PAIRED NEIGHBOUR: a module-scope IMMUTABLE PLAIN-DATA CONSTANT stays GREEN, proving the rule rejects injected STATE and not every module-scope reference** | §2 requirement `1b` closed export/state surface — ⚠️ **NOT the import-graph check, which stays silent here** |
| **35** | **clean control — unmutated** | **GREEN**, with the frame, resolved scope and digest printed | all |
