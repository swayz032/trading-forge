...
> 5. ★★★★★ **A WHOLE-EXPECTATION-SURFACE MUTATION: perturb EVERY expectation in the ledger at once; the required result is an IDENTICAL PROJECTION DIGEST.** ⚠️ **This is the one that does not care HOW the coupling was formed** — signature, import, closure or cache — **it only asks whether the output moved.** `A CAPABILITY ARGUMENT CAN BE WRONG ABOUT THE MECHANISM; A DIGEST THAT DID NOT MOVE CANNOT.`
...
> ### ★★★★★ CONTRACT 2b — **A CLOSED DESTINATION WIRE SCHEMA**, VALIDATED BOTH DIRECTIONS
> **The frozen wire schema is exactly `{bindable, session_zone, approximation, primitive, reason}`.** After projection, **every emitted destination key must be a MEMBER of that set, and every member must be PRESENT** — unknown destination and missing destination are both RED and both NAME the key.
> ⚠️ **This is a DIFFERENT boundary from `projectExhaustively()`'s source-key check, and the design keeps BOTH:** the source check defends the mapping's INPUT (an unmapped raw field), the destination schema defends its OUTPUT (a mapping that renames a field to something nobody consumes). ★★★ **The `sessionZone` rename is invisible to the first and fatal to the second, which is precisely why one cannot stand in for the other.**
...
⚠️★★★★★ **THIS MATRIX IS `29` MUTATIONS **PLUS** `1` CLEAN CONTROL — `30` NUMBERED ROWS.** `[COUNTED FROM THE TABLE BELOW BY PARSE, after the rows landed; NOT copied from any prior statement of the figure.]`
★★★★★ **THE CAPTION IS STATED IN THAT FORM BECAUSE IT HAS ALREADY BEEN WRONG TWICE IN THE WAY THAT FLATTERS A SUITE: `AR-566`'s header said *"`23` MUTATIONS"* when the truth was `22`+`1`, and the previous revision of THIS line said `24`+`1`, which was true only until five rows landed.** ⚠️ **`A CONTROL COUNTED AS AN ATTACK INFLATES A SUITE'S ADVERSARIAL STRENGTH BY EXACTLY THE ROW THAT PROVES IT IS NOT ALWAYS RED`, and `A COUNT CARRIED ACROSS AN EDIT IS STALE THE MOMENT THE EDIT LANDS.`**
⚠️★★★ **RE-DERIVED WHENEVER A ROW IS ADDED — never carried, including from this document's own previous revision.** `A HAND-COPIED EXPECTED VALUE IS A FABRICATED SAFETY CLAIM.`
...
| **27** | ★★★★★ **UNIQUE-WRONG-DESTINATION RENAME** — `sessionZone: "session_zone"` → `sessionZone: "sessionZone"` (source set identical · destination unique · entry consumed) | RED — **unknown destination key NAMED** | §2b closed destination wire schema — ⚠️ **NOT `projectExhaustively()`, which stays silent here** |
...
| **30** | **clean control — unmutated** | **GREEN**, with the frame, resolved scope and digest printed | all |
