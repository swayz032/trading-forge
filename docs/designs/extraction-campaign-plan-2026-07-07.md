# Extraction — Campaign Plan (decision tree, not a march) 2026-07-07

Not a path to a known summit — a two-hypothesis experiment with pre-registered pivots. Every phase advances OR cleanly falsifies; no phase's success bar is set after its predecessor's numbers exist. The shape is the point: a plan that cannot lie about whether it's working.

## Phase 0 — Clear the owed debt (registered tier, ~now). No decision point; debt service.
Defects 7/8/9 land; Defect-9 materiality resolves; real-corpus Defect-10 receipt; engine-semantics seam ruled. Then the re-baseline: null-cal → Mode A/B on the fully-corrected engine, tripwire pre-armed (read-order from the Defect-10 ruling). Nothing downstream is trustworthy until the JUDGE is re-certified post-surgery.

## Phase 1 — H1: build the fidelity instrument the gradient taxonomy specced.
The frontier. Design brief (handed by the week):
- (a) deterministic detection of the two carrying surface classes — **conditional-action** ("buy when retested") + **exclusion-contrast** ("X, not Y");
- (b) route narration / probabilistic surfaces to a **discourse-aware adjudication pass** rather than guessing (canonical Fibonacci: the sentence goes dark, context carries);
- (c) emit a **fidelity certificate** per strategy (gate-strength survived blind adjudication under the closed taxonomy, control-gated rater, quote-anchored).
...
- **H2 falsified:** the pivot executes without mourning — engine, certificates, adjudication machinery, taxonomy all TRANSFER INTACT to the next source population (books, papers, event-anchored/microstructure hypotheses). The beauty of building the JUDGE before betting on the DEFENDANT.
...
2. **Separate MISSION-success (absolute) from CONVEYOR-economics (rate) — they are different questions and must not collapse.**
   - **Mission read:** **≥1 robust survivor** (passes the full anti-overfit battery: PBO/DSR/WFE/B14/B15) = the mission HAS its scaleable strategy. This is primary — the mission needs ONE, not a rate.
