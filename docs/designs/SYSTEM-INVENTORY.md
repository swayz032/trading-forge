...
> Generated at commit `254b876db4b6a0ff4a7706c798475c8efa7a6de0`  (worktree DIRTY at generation time)
...
|---|---|---:|---:|---:|---:|
| `src/` | Python | 309 | 402 | 127627 | 1982 |
...
| `src/` test files | 1127 | tests are the reference surface, never the symbol surface |
...
Total entry points: **187**.  Modules reachable from them: **733** of **2169** parsed files.
...
| C3 | BUILT-UNREACHABLE is reachable by the classifier | PASS | BUILT-UNREACHABLE=1196 |
| C4 | result is not uniform (broken-probe tell) | PASS | largest bucket = 74.8% of 4965 rows |
...
| C12 | symbols enumerated in both languages | PASS | py=1982 ts=2933 |
...
* **Name collision biases toward `WIRED` - and the affected population is MEASURED, not
  ...
  **221 of 4644 enumerated symbol names (4.8%) are defined in more than one file, covering
  492 of 4915 symbol rows (10.0%).**  Every symbol table below marks those rows `AMBIG`.
...
| `src/engine/extraction` | 262 | 0 | 74 | 0 | 0 | 336 |
...
Of **1196** `BUILT-UNREACHABLE` symbols, **644 have test coverage but no production caller**.
...
| `finalize` | function | `src/engine/extraction/g2d_finalizer.py:86` | 4 | unique |
| `freeze_isolated_queue` | function | `src/engine/extraction/isolated_fallback_law.py:100` | 4 | unique |
...
| `DurableAttemptLedger` | class | `src/engine/extraction/isolated_attempt_receipt.py:71` | 3 | unique |
...
| `collect_isolated_results` | function | `src/engine/extraction/g2d_finalizer.py:57` | 1 | unique |
...
_...344 more omitted from this table._
...
<details><summary><code>src/engine/extraction</code> - 74 symbols</summary>
...
| `collect_isolated_results` | function | `src/engine/extraction/g2d_finalizer.py:57` | no non-test reference outside its own definition; 1 test file(s) do reference it |
| `finalize` | function | `src/engine/extraction/g2d_finalizer.py:86` | no non-test reference outside its own definition; 4 test file(s) do reference it |
