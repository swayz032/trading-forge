...
## OPERATIVE-PATH TRACE RESULT (2026-07-09) — CLEAN SYMMETRIC → Mode A/B authorized (pre-committed routing, no round-trip).
Engagement-evidenced (real Band-C CLI probes, not code-reading). ★ The trace CORRECTED THE PARENT: my resume message fabricated a "session-limit cutoff" + attributed pre-existing repo debris (tmp-strats.json) to the agent; it disregarded the incorrect framing + answered from fresh probe evidence. Blade points at the parent too — recorded.
- **Q1 (load-bearing): NO macro path fires for either exam arm — STRUCTURALLY.** Both arms = Band-C dispatch (`run_class_backtest`); `main()`'s Band-C branch NEVER forwards `event_calendar`. → Path B (Part-A mirror) UNREACHABLE (not just unconfigured, `event_calendar is None` always); Path C (fallback) STRUCTURALLY ABSENT from run_class_backtest (exists only in run_backtest/DSL); Path A (eligibility overlay) engages but carries NO macro logic (ICT A+ only, skip_reasons all non-macro). Probe: demotion-arm 1,727 trades `passthrough_strategy_unregistered` macro-blocked=0; classifier arm identical by construction.
- **Q2: SYMMETRIC, no confound.** Both arms equally-unregistered names → both bypass the (macro-free) overlay identically. Residual (flagged, not routed): `{family}_{sym}_{tf}` names ARE in ALL_STRATS but scripts use them only as JOIN KEYS, never as engine strategy_name — doesn't touch macro.
- **Q3: MOOT** (no reachable path). Dormant Path B (if wired) = Part-B calendar / timestamp ±30min / Ruling-A scoping (already traced) — none engages.
- **Q4: reference RE-DERIVED IN-RUN on G4** (corpus-v3-reference-rederivation.py → this worktree's run_class_backtest, ancestor of cfa0cb3), not pre-Part-B cited. Calendar-backfill moot (macro never engages).
- **Q5: null arm's Path C fallback was on run_backtest (DSL) — a DIFFERENT engine fn than the exam arms (run_class_backtest/Band-C).** Cannot infer exam-arm macro exposure from it; and its trade-removal only made nulls MORE likely to fail → floor stays conservative + valid.
- **ROUTING: both arms same path (none) same semantics (structurally zero) → COMMON-MODE → Mode A/B proceeds unmodified.** No confound, no date-flattening, no cross-gen. Option B stays rejected (don't inject event_calendar into a frozen exam / certify a config production doesn't run for Band-C).
- **SUPPRESSION RECEIPT = N/A** (operative path doesn't consume Part B's calendar; mask dormant, defense-in-depth tier — the pre-committed resolution of the latent receipt-vs-tier contradiction; tier wins).
- **★ CONCRETE ADDITION beyond F-4:** Path-B dormancy isn't just "no one passes policies" — `main()`'s Band-C branch has NO code capable of forwarding event_calendar to run_class_backtest even if config had it. The macro activation packet must add that THREADING in main(), not just populate config JSON.
- **CERTIFICATION SCOPE (honest):** the re-baseline certifies the engine with the corpus (Band-C) route engaging ZERO macro-blackout, both arms. Macro-mask engagement is deferred to the activation packet. Scope travels with the certificate: "macro handling = absent for the Band-C corpus route."

---

## 0.5-EXAM AFFIRMATIONS + FIFTH META-RULE PRE-REGISTRATION (2026-07-09).
Null-cal ACCEPTED as stage-1 read (0/100 full battery, DSR/WRC/SPA doing the killing; wf_cpcv+PBO passing individually kept visible — PBO weakly informative on degenerate strategies, floor's power is the CONJUNCTION; mask-agnostic, null-arm fallback only conservative). Trace ACCEPTED (ethos note: subagent answered from own probe evidence + corrected the parent's framing incl. reclassifying parent-handed tmp-debris — doer≠grader holding at the layer it erodes first). Phase-0 debt-service COMPLETE as tabled.

### ★★ FIFTH META-RULE APPLICATION — pointed at Mode A/B's OWN TREATMENT VARIABLE (the overlay). "The exam measures the overlay's value" requires "the overlay ACTUALLY ENGAGES in Mode B." The trace showed exam-artifact specs hitting `passthrough_strategy_unregistered` (overlay BYPASS); if any of the 78 corpus specs resolve UNREGISTERED at eligibility, Mode B silently degenerates to Mode A → A/B measures nothing → corpus-wide engagement failure would read as "overlay effect vanished" (false finding from feed failure). PRE-REGISTERED before any verdict:
1. **Launch pre-check** (DS#21 audit-unregistered-strategies.ts / eligibility-registration.ts against the 78 exported NAMES; expected 0/78 unregistered per standing 0-of-120). Eligibility keys on the strategy NAME ({family}_{sym}_{tf} in ALL_STRATS), NOT spec_id — export must use the registered-name form or overlay bypasses. Any hit → kill/fix-naming/relaunch (minutes vs multi-hour); relayed to the launch agent with authority to self-correct.
2. **Per-spec engagement evidence**: Mode B reports overlay-engagement counts, passthrough=0; any passthrough-in-Mode-B spec = invalid A/B pair, excluded not averaged.
3. **Zero-diff tripwire**: Mode A ≈ Mode B corpus-wide = presumptive ENGAGEMENT FAILURE (not "overlay stopped mattering") → stop verdict, route to trace.
4. **Pairing integrity**: same 78/window/config, only mode toggle; comparison consults mode-ab-guard hash discipline.

### ★★ 0.5-VERDICT READING ORDER (pre-committed, mirrors campaign law): VALIDITY BLOCK FIRST (engagement evidence + pairing integrity + tripwire state) → THEN directional read vs G0 → THEN classifiability. **NO GOALPOST ON DIRECTION**: G0 says overlay HURTS; G4 confirms=frozen conclusion stands on corrected engine / G4 flips=legitimate finding (old direction partly a broken-engine artifact, feeds corpus-v3 arc) / mixed=records as such. **MAGNITUDES NOT CROSS-GEN COMPARABLE** (G0→G4 spans guard fixes + dormancy alignments) — frozen criterion is directional FOR THAT REASON; verdict prose quoting cross-gen magnitude deltas as findings overreaches the instrument. What to DO about the overlay's frozen status afterward = separate decision back through operator.

### POST-0.5 QUEUE (in order, none moves until the 0.5 read is in): pass-3 N=9 reference re-derivation (registered, downstream) → Gate-3 classifier read under ≥(N−1)/N → post-campaign families (dormant-defaults census + activation children [fills, VIX, macro] + direction-coverage trace).

---

## MODE A/B VALIDITY PRE-REGISTRATIONS #1-3 (2026-07-09, folded in while battery grinds).
### 1. MCL exclusion FORMALIZED (G0-parity) — exact boundaries:
EXPECTED: exactly 26 MCL-15min INDETERMINATEs, same data-gate signature (46 zero/neg prices), firing BEFORE overlay logic, byte-identical to G0's aborts. BOUNDARIES: any MCL spec failing DIFFERENTLY, or any NON-MCL spec hitting the data gate = NEW anomaly → TRACED, not absorbed into the exclusion. CONSEQUENCE (result-claim-scoping): the 0.5 verdict is scoped to the **~52-spec MES/MNQ effective corpus — crude UNTESTED, in G4 exactly as in G0.** Every downstream citation carries the scope; H2 language never says "the corpus" unqualified.

### 2. MCL DIAGNOSTIC — SETTLED: the 46 zero/neg prices = the APRIL-2020 WTI NEGATIVE-PRICE EVENT, not corruption.
Grep of `data_cache/CL/ratio_adj/15min.parquet`: all 46 bad bars in 2020-04 — FIRST `2020-04-20 14:00 ET`, LAST `2020-04-21 09:15 ET` (WTI settled −$37.63 on 2020-04-20 + next morning). Ratio-adjusted continuous contracts are mathematically ill-behaved through a negative-price episode (ratio adjustment assumes positivity) → this is the event's ratio-adjustment SHADOW, a historical fact. ★★ STANDING WARNING PINNED: the fix is NEVER clamping/interpolating prices (fabricating data) — a future agent flooring CL at $0.01 would poison every crude backtest forever. Real options = POST-CAMPAIGN data-lane decision: truncate the crude corpus window past mid-2020, OR difference-adjust (additive) instead of ratio-adjust for CL. [[reference_mcl_april2020_negative_price_not_corruption_2026_07_09]].

### 3. ★★ SIXTH META-RULE APPLICATION — decision-variance (the engagement pre-reg's missing half).
Stamps prove the overlay CODE RUNS; they do NOT prove it DECIDES anything. `tf_institutional_overlay` + passthrough=0 establishes Mode B CALLS the overlay — but an overlay whose INPUTS are absent in class WF (bias context, playbook state) could evaluate "allow everything" every bar → bit-identical A/B with perfect engagement stamps = the feed-void ONE LEVEL DEEPER (overlay's own inputs). PRE-REGISTERED: the validity block includes, per spec, the count of entries the overlay actually SUPPRESSED/ALTERED in Mode B — **trade-count delta A-vs-B is a sufficient proxy** (data confirmed present: manifest mode_a/mode_b carry `trade_count`). EXPECTED NONZERO corpus-wide — NOT a guess: G0's "overlay HURTS" means G0's overlay demonstrably ACTED. ZERO decision variance across all ~52 effective specs = cross-generation engagement change → **validity block FAILS → read STOPS for an input-feed trace**; no directional language over an overlay that never acted. Zero-diff tripwire + this = ONE mechanism at TWO grains (corpus-identity=engagement-failure; per-spec-variance=where).

### VALIDITY BLOCK (complete, pre-committed order): pre-check(0/78 ✓) → engagement stamps(passthrough=0) → DECISION VARIANCE(nonzero per-spec trade-count delta) → pairing integrity(same 78/window/config, mode-ab-guard hash) → tripwire state(MES/MNQ scoped) → MCL exclusion(26-count + signature + April-2020 diagnosis). THEN direction vs G0 (only if validity clean) → classifiability. Magnitudes NEVER cross-gen. .env line-46 placeholder → hygiene list (broken POSIX source silently eats env vars future feeds depend on).

---

## MODE A/B EPOCH ACCOUNTING (2026-07-09/10) — verified for the validity block.
Incidents between relays (sizing fix + freeze + relaunch PID 18991→363, workers 3→2) checked for epoch boundaries:
- **ONE engine SHA across all 78.** backtester.py=8501 (G4), `git status` zero uncommitted engine edits, wt-parta HEAD advance cfa0cb3→00c2618 was DOC-ONLY (null-cal report + trace record, no engine bytes). Single engine, start to finish.
- **Fix was engine-CODE-untouched (no G4′)** — `git diff` on engine files empty. AND engaged-config dimension resolved: **`TF_ALLOW_FIXED_1=true` (config.py:414-432) SUPPRESSES the H7 fail-fast validation that rejects the default `position_size.fixed_contracts=1`; it changes NO sizing behavior** (specs run at 1 contract either way; the flag only permits-vs-errors), and the 1-contract sizing is COMMON to both A/B modes (cannot confound the overlay comparison). conftest.py sets it true by default (standard backtest posture). Applied uniformly (single launch env, pre-fix rows deleted) → uniform-within-G4, common-mode within A/B.
- **G0-comparability of the flag = STORY (a), parity-restored.** The flag-rescued specs lack explicit sizing → fall to the fixed_1 default. They have real G0 counterparts: the G4 strategy-list IS the 78 G0 spec_ids (built from the G0 manifest); G0 ran ALL 52 non-MCL CLEAN (trade_count present). So G0 ran these exact specs (same flag or pre-H7-validation) → the flag RESTORES G0-comparable behavior, NO new-in-G4 orphans, every non-MCL spec has a G0 row, **effective N≈52 unchanged.** Story (b) [G0 dropped them] RULED OUT (G0 ran 52/52 non-MCL).
- **No resume-trap.** Relaunch used NO `--resume` + freshly-cleared manifest; first row generated_at 2026-07-10T00:52Z (post-freeze); all rows one PID-363 epoch. The null-cal error-record-as-completed trap cannot apply — nothing carried over.
**Conclusion: the freeze+relaunch created NO epoch boundary in results. Single-engine (G4), single-epoch, uniform-flag, G0-parity-preserved run.** Folds into the validity block next to engine SHA. EARLY DECISION-VARIANCE (6th-app check): 13 real pairs, Mode A ~600-700 trades vs Mode B ~130-180 → overlay cuts ~75-80% → strongly nonzero, overlay demonstrably ACTS (not a stamps-but-no-decisions feed void).

---

## TRIPWIRE RESOLVED (2026-07-10) — 7 G0→G4 zero-trade flips = the OR-BRANCHES HONORING FIX (2026-07-05, already-receipted). Directional read UNSEALED.
Pre-registered zero↔nonzero tripwire fired on 7 specs (traded in G0, zero in G4, identical windows/exec-path). VERIFICATION-BEFORE-ALARM: initial "12/12 flips" caught as a broken join (G0 uniform trade_count=8), verified + stripped → 7 real flips survived. CAUSE TRACE (pre-committed order): H1 guard-suppression RULED OUT (flipped spec ran on G4 = 0 trades, ALL guard counters 0, guards_failed=false → not suppression, signal-generation). Localized to signal-gen → OR-branches lead.
- **CAUSE = OR-branches honoring fix (`docs/designs/or-branches-honoring-fix-2026-07-05.md`), landed dead-center in the G0→G4 delta.** All 7 flipped ids in its CCR receipt (7/7, unhonored-OR domain: expected 3/3/11 executed=0). Mechanism = the fix's TWO DEFAULT-PATH (non-flag-gated) direction-correctness bugs: `_eval_wait_bias` hard-coded want_bearish=False (every WAIT_BIAS forced bullish) + confirmation OR-blended away direction. `TF_OR_BRANCHES_ENABLED` absent from .env → default OFF → flag-gated OR-honoring INACTIVE → the flip is purely the unconditional direction fixes.
- **ITEM 1 per-pair evidence (self-contained):** all 7 flipped specs carry `wait_bias=True AND confirm=True AND or_branch=True` (their own config fields, not CCR membership by proxy). Tested spec (bos_and_fvg_or_fvg_mes_15m, bullish-only) confirmed: G0 forced-bullish WAIT_BIAS → 8 bug-fires; G4 real-direction → 0. G0's 8 = mis-evaluation; G4's 0 = corrected instrument.
- **CLASSIFICATION (pre-committed): expected fix-working.** 7 pairs join the zero-trade class with stated cause; validity block carries per-pair trigger evidence; **directional read PROCEEDS. G0 side gets the annotation (old engine mis-evaluated direction), NOT G4.**
- **ITEM 2 — exactly-8 signature PRE-REGISTERED (before clustering check):** forced-bullish WAIT_BIAS resolves the wait INSTANTLY → each G0 fire sits at the first evaluable bar(s) of each WF window, one-shot per window reset × 8 windows = exactly 8. If the 44-spec exactly-8 population's G0 trades cluster at window-start bars → whole exactly-8 mystery closes under this mechanism, G0 baselines confirmed substantially bug-fires. Clustering check folds into the validity block (confirms/falsifies the prediction, doesn't decorate).
- **ITEM 3 — SCOPE LINE:** OR-composites evaluated under UNHONORED-OR semantics in BOTH generations (flag OFF in G0 and G4 → common-mode, comparison honest). Scope stamp: "OR-composites under unhonored-OR semantics, both generations." FORWARD (engaged-config law): flipping `TF_OR_BRANCHES_ENABLED` someday = an ENGAGED-CONFIG EPOCH → corpus re-baselines for OR-specs when honoring turns on. Written now so nobody flips it casually.
- **ITEM 4 — this battery IS the missing count-receipt, 78-wide.** The direction fixes are UNCONDITIONAL → blast radius = every WAIT_BIAS/confirm-carrying corpus spec. This run's G0→G4 per-spec delta table documents exactly that (the count-coverage gap the fix shipped with is closed by running this battery).
