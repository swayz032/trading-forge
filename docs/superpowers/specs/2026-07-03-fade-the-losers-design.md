...
**Statistical honesty:** fading doubles the hypotheses tested (selection), so the fade does NOT get a lighter battery — same rigor, or the survivors are just data-snooped noise. Faded strategies are tagged (`source='fade_the_losers'`). **Accurate scope of that tag (corrected after review):** the tag is **provenance-only** today — `corpus-fdr-report.py` has no `source` filter, so a fade enters the corpus-FDR multiple-testing correction **incidentally via its backtest**, exactly like any other strategy; it is NOT yet counted as its own distinct selection-family. Because every fade still traverses the full unchanged battery (CPCV/WFE/PBO/DSR/B14/BIF + slippage gate), a fade survivor has already passed what every other strategy passes. **Future enhancement (not built here):** teach `corpus-fdr-report.py` to read `source='fade_the_losers'` and group fades as their own testing-family so the selection-doubling is corrected explicitly. Do not claim tag-driven FDR that does not exist yet.
