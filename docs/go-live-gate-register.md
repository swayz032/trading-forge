...
- **C4 survival-score gate structurally inert — FIXED 2026-07-18, commit `59727627`.** `raw_survival_score` was unconditionally 0.0 because every `backtester.py` call site passed `survival_results=None` to `compute_forge_score()`. Fixed via a new shared helper, `_compute_survival_results_for_gate()`, wired into both production call sites (`run_backtest()`, `run_class_backtest()`): scores against the strategy's pinned firm (`BacktestRequest.firm_key`, e.g. `"topstep_50k"`) if set, otherwise conservatively against BOTH canonical firms (Topstep + MFFU) and takes the worse score. Fails open (returns `None`, degrades to the exact pre-fix behavior) on any internal error. New test coverage: `src/engine/tests/test_c4_survival_gate_wiring.py`. The one deliberately-untouched surface: a third, CLI-only stress-test recompute call site (`main()`, not part of the production TS↔Python subprocess path) — noted, not wired, disproportionate plumbing for its production relevance.
