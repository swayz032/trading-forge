...
**★ Daily dollar figures are OBSERVED OUTCOME ranges — never quotas and never ceilings (operator directive 2026-07-16).** There is NO fixed take-profit: the runner trails (structure/POC/Chandelier) until invalidation or the 15:55 ET flatten, so a strong trend day at Phase-1 size can exceed $1,000 — nothing anywhere caps a winning day, and nothing forces a $250 minimum on a no-quality day (0 trades = a valid day). Any future code/doc that reads a daily dollar figure as a target or cap is regressing this directive (the shadow profit-governor, when built, is observability-only and may never touch an exit).
...
| `deep-scan` | Any deep-scan / audit / fix wave / re-certification — BEFORE dispatching finder or fixer agents. Phases 0–6: pin+scope → find → verify-findings → fix (worktree + ratify-packet for instrument code) → certify → land → record. Zero-carry-forward default. |
...
**Scheduler cron notes (Wave 27 Pass 1.5):** `quantum-replay-weekly-analysis` fires Sunday 19:00 ET (DST-safe double-fire at 23:00 UTC with ET-hour=19 guard) and emits a SIGNAL / INCONCLUSIVE / NO_SIGNAL / PRELIMINARY verdict to Discord (and `docs/replay-results/<ISO>-quantum-disagreement.md` via `scripts/replay-grade-quantum.ts`). FAILURE state emits a critical alert. Kill switch (3-mode numeric): the weekly cron is an AUTONOMOUS loop, so it runs ONLY at AUTOPILOT (`current_value='2'`). `UPDATE system_parameters SET current_value = '0' WHERE param_name = 'auto_patch_loop_enabled'` halts the cron's mutation path (OFF); `'1'` (OBSERVE) also halts autonomous mutation while keeping nightly advisory intelligence; `'2'` (AUTOPILOT) is the only mode that runs it. Same row also gates Wave 26 `pattern-aggregator`. Semantics owned by `src/server/lib/learning-loop-mode.ts`.
