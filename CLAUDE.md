...
### Take Profit — Adaptive (Wave 25 Pass 7 — SCAFFOLD, awaits Wave 25.5 wiring)

`adaptive-exit-engine.ts.computeExitPlan()` is shipped and tested as a pure function. **Not yet wired into the production signal flow** — Wave 25.5 closes the 3 integration gaps (paper-signal-service position-open, backtester _apply_trade_management branching, runner trail execution). Until Wave 25.5 ships, Style C below remains the LIVE default for all strategies regardless of `exit_plan_config.exit_style` setting.

When Wave 25.5 ships, adaptive will be:
- TP1: liquidity-mapped (intraday DOL only — PWH/PWL/PMH/PML excluded per day-trader mandate) OR +1.0R fallback
- TP2: next intraday liquidity OR +2.0R fallback
- Runner trail: regime-selected (anchored_vwap / developing_poc / chandelier / structure_trail)
- Scaling: regime-dependent (TRENDING 20/30/50, RANGE 50/30/20, MACRO 60/30/10, CHOP 50/50/0)
- Early-exit: cumulative-delta divergence ≥ 0.6 + position in favor ≥ 0.5R → 25% partial close (prop-firm-safe, never flips)
- Pre-lunch: RANGE/CHOP/COMPRESSION at 11:30 ET ≥0.3R → 50% partial + BE+0.5R stop
- 15:55 ET hard flatten INVARIANT preserved

### Take Profit — Style C (Wave 23 canonical — CURRENT LIVE DEFAULT)
...
| **Wave 25 Pass 7 — exit_plan_config.exit_style switch** | strategy persistence | exit_style="static_styleC" default preserved; exit_style="adaptive" valid but scaffold-only until Wave 25.5 ships the 3 wiring gaps |
...
- **Don't claim adaptive exits are live default until Wave 25.5 ships.** Pass 7 shipped the engine + scaffold + tests; the position-management loop still executes Style C until paper-signal-service.ts + backtester._apply_trade_management() + updatePositionPrices() are wired.
