...
- **Don't change `transcript_extractor` prompt or KB cards without re-running 5-fixture parity test (Wave 26 gemma4 routing)** — gemma4:e2b quality is entirely prompt-dependent. The v9 strict-fill prompt + KB cards (strategy-schema-snapshot.json + indicator-catalog.md) + few-shot examples (fix 15 + fix 16) form the calibrated extraction surface. Changing any of these requires re-validating W23H field coverage (bias_timeframe / confirming_indicators / preferred_regimes) on at least 5 representative transcripts. Prompt is at `src/agents/transcript-extractor.md` — treat as read-only between parity tests.
...
- **AI Agents:** TypeScript + Ollama (qwen2.5-coder:7b primary, deepseek-r1:14b, gemma4:e2b for transcript_extractor) + GPT-5-mini (cloud fallback). Pass 21 (2026-05-12) retired qwen3-coder:30b — 18GB model couldn't load on RTX 5060 8GB VRAM. Wave 26 local-first swap: `transcript_extractor` primary = `gemma4:e2b` (Ollama); gpt-5-mini is now the FALLBACK for that role only. Requires Ollama ≥ 0.4.x for gemma4 support. Override via `TRANSCRIPT_EXTRACTOR_LOCAL_MODEL` env var; panic-revert via `TRANSCRIPT_EXTRACTOR_FORCE_CLOUD=true`. Override other models via `PARAMETER_EVOLVER_MODEL` env var.
