...
**Deep-Scan #19 B-3 env var (exit-side volume-impact fill-price degradation, 2026-07-05):**
- `BACKTEST_EXIT_PARTIAL_FILL_ENABLED` (default `"false"` — opt-IN, opposite polarity from `BACKTEST_PARTIAL_FILL_ENABLED`) — activates `src/engine/fill_model.py::apply_exit_volume_price_impact()`, the symmetric exit-side counterpart to the entry-side volume partial-fill model. `apply_volume_partial_fills()` only ever degrades ENTRY bars (backtester.py:4491/4570 pass the entry array) — exits (stop-outs, TP fills, 15:55 ET hard flatten) always filled at the idealized price, overstating exit quality on thin bars. Since a closing order can never be partially filled without leaving risk open, this model degrades the fill PRICE (up to `MAX_EXIT_IMPACT_TICKS=2.0` ticks, same 3-zone qty/bar-volume-ratio shape as the entry model, reusing `BACKTEST_PARTIAL_FILL_VOLUME_THRESHOLD` for the zone-2 onset) instead of the size. Wired into `_apply_static_styleC_management()`'s stop-loss/trailing-stop and 15:55 time-stop fill-price assignments (`src/engine/backtester.py`) — the 15:55 ET hard flatten INVARIANT is preserved exactly: this flag can only make its fill price worse, never gate whether/when the flatten fires. Default FALSE means byte-identical behavior to every historical backtest until the operator opts in (same discipline as `BACKTEST_STRUCTURAL_STOP_PARITY_ENABLED`). TP1/TP2 fill-price degradation is NOT yet wired (those static +1R/+2R levels double as the intrabar trigger threshold; degrading them there would also shift trigger timing, not just fill price — deferred as a follow-up requiring a trigger/fill price split). When you flip it ON, historical backtests with thin-volume exit bars become non-comparable — re-run before trusting metrics for promotion.
