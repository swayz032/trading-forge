//@version=1
// Slumdawg FX Replay V2.0.2 — 15m Current Move + Entry + Deep TP
...
// v0.2 critical fix: keep reaction history by trade side. A crowded set of
// reactions above the Entry Zone cannot exhaust the short-side history budget.
...
const mergeLadders = (side, entry, entryGap, zoneGap, lists) => {
  ...
  for (let n = 0; n < 3; n++) {
    let best = null;
    for (const p of rows) {
      const ok = side === "LONG" ? p >= boundary + gap : p <= boundary - gap;
      if (!ok) continue;
