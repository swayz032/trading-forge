...
class ProofSelectorTests(unittest.TestCase):
    def test_countertrend_rejects_nearest_tiny_wick(self):
        tiny = ProofCandidate(
            ...
            distance_normalized=.2,
            room_to_target_normalized=2.0,
            structural_score=.3,
        )
        meaningful = ProofCandidate(
            ...
            distance_normalized=1.4,
            room_to_target_normalized=2.0,
            structural_score=.85,
        )
        d = select_proof_level(
            [tiny, meaningful],
            Direction.LONG,
    ...
    def test_rejects_impractically_far_level(self):
        far = ProofCandidate(
            ...
            distance_normalized=5.0,
            room_to_target_normalized=2.0,
            structural_score=.95,
        )
        d = select_proof_level(
            [far], Direction.LONG, OverallDirection.BEARISH, PROOF_CFG
        )
    ...
    def test_countertrend_requires_more_structure_than_withtrend(self):
        c = ProofCandidate(
            ...
            distance_normalized=1.2,
            room_to_target_normalized=1.5,
            structural_score=.60,
        )
        counter = select_proof_level(
            [c], Direction.LONG, OverallDirection.BEARISH, PROOF_CFG
        )
        withtrend = select_proof_level(
            [c], Direction.LONG, OverallDirection.BULLISH, PROOF_CFG
        )
    ...
    def test_higher_timeframe_wins_true_tie(self):
        ...
        d = select_proof_level(
            [m5, h4], Direction.LONG, OverallDirection.BEARISH, PROOF_CFG
        )
