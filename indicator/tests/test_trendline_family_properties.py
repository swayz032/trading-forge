...
class TrendlineFamilyPropertyTests(unittest.TestCase):
    def test_randomized_family_lineage_and_frozen_geometry_1000_paths(self):
        for seed in range(500):
            for direction in (TrendDirection.BULLISH, TrendDirection.BEARISH):
                ...
                for line in fam:
                    ...
                    self.assertEqual(line.signature, before)
                for parent, child in zip(fam, fam[1:]):
                    self.assertEqual(child.anchor_a, parent.anchor_b)
                    self.assertEqual(child.parent_line_id, parent.line_id)
                    self.assertEqual(child.parent_revision, parent.revision)

    def test_randomized_repair_never_changes_nonviolated_slots_500_paths(self):
        exercised = 0
        for seed in range(500):
            ...
            adverse1 = (
                projected1 - 1.0
                if victim.direction == TrendDirection.BULLISH
                else projected1 + 1.0
            )
            adverse2 = (
                projected2 - 1.0
            ...
            self.assertEqual(broken.state, LineState.VIOLATED)
    ...
    def test_duplicate_and_out_of_order_close_events_do_not_double_count(self):
        ...
        self.assertEqual(one.breach_streak, 1)
        ...
        self.assertEqual(older, one)

    def test_repair_preserves_hidden_visibility_ids(self):
        ...
        self.assertEqual(repaired.hidden_line_ids, board.hidden_line_ids)
