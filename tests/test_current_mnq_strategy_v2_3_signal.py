...
from research import current_mnq_strategy_v2_3_signal as sig
...
def test_completed_inputs_physically_remove_unclosed_1m_and_5m_bars():
    ...
    assert list(f.index) == list(idx5[:2])  # 09:30 and 09:35 bars are completed
    assert list(o.index) == list(idx1[:10])  # through the 09:39 minute
...
def _patch_candidate(monkeypatch, direction: str, actionable: pd.Timestamp):
    ...
    cand = sig.core.Candidate(
        ...
        signal_time=actionable - pd.Timedelta(minutes=5),
        confirmed_time=actionable, reason="STRONG_5M_ACCEPTANCE",
    )
    plan = SimpleNamespace(
        primary="BULL" if direction == "L" else "BEAR", score=1.0,
        pm_structure="TREND", location_state="ROOM",
    )
    monkeypatch.setattr(sig, "_first_candidate", lambda env, dte, p, asof: iter([(cand, actionable, plan)]))
...
def test_fresh_long_binds_ask_not_bid(monkeypatch):
    d = date(2026, 8, 17)
    actionable = pd.Timestamp("2026-08-17 10:05", tz="America/New_York")
    ...
    assert decision.stop == 83.0


def test_fresh_short_binds_bid_not_ask(monkeypatch):
    d = date(2026, 8, 17)
    actionable = pd.Timestamp("2026-08-17 10:05", tz="America/New_York")
    ...
    assert decision.stop == 117.25


def test_old_first_setup_uses_historical_reference_and_is_not_fresh(monkeypatch):
    d = date(2026, 8, 17)
    actionable = pd.Timestamp("2026-08-17 10:05", tz="America/New_York")
