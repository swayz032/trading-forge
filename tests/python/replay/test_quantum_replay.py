...
def _make_mock_db_loader(
    backtest_ids: Optional[list[str]] = None,
    daily_pnls: Optional[list[float]] = None,
    classical_ruin: Optional[float] = None,
    wf_windows: Optional[list[dict]] = None,
    strategy_id: Optional[str] = None,
) -> MagicMock:
    """Build a mock db_loader with configurable return values."""
    m = MagicMock()
    m.load_backtest_ids.return_value = backtest_ids or [FAKE_BACKTEST_ID]
    m.load_daily_pnls.return_value = daily_pnls if daily_pnls is not None else SYNTHETIC_DAILY_PNLS
    m.load_classical_ruin.return_value = classical_ruin if classical_ruin is not None else FAKE_CLASSICAL_RUIN
    m.load_walk_forward_windows.return_value = wf_windows or [
        {"id": FAKE_FOLD_ID, "window_index": 0, "is_start": "2024-01-01", "is_end": "2024-06-30"}
    ]
    m.load_strategy_id.return_value = strategy_id or FAKE_STRATEGY_ID
    m.write_replay_row.return_value = None
    return m


# ── Helper: patch db_loader inside quantum_replay module ─────────────────────

def _patch_db(mock_loader: MagicMock):
    """Return a context manager that patches _db_loader and _DB_LOADER_AVAILABLE."""
    return patch.multiple(
        "src.engine.replay.quantum_replay",
        _db_loader=mock_loader,
...
class TestDryRunIsolation:
    def test_dry_run_no_db_writes(self):
        """apply=False must never call write_replay_row."""
        mock_loader = _make_mock_db_loader()
        with _patch_db(mock_loader):
            results = replay_quantum_on_backtest(
                backtest_id=FAKE_BACKTEST_ID,
                apply=False,
                seed=DEFAULT_SEED,
            )
        mock_loader.write_replay_row.assert_not_called()
    ...
    def test_dry_run_all_backtests_no_writes(self):
        ...
        mock_loader = _make_mock_db_loader(backtest_ids=[FAKE_BACKTEST_ID])
        with _patch_db(mock_loader):
...
class TestApplyMode:
    def test_apply_calls_write_for_each_completed_row(self):
        """apply=True calls write_replay_row once per completed (fold, event_type) tuple."""
        n_folds = 2
        wf_windows = [
            {"id": f"fold-{i}", "window_index": i, "is_start": "2024-01-01", "is_end": "2024-06-30"}
            for i in range(n_folds)
        ]
        mock_loader = _make_mock_db_loader(wf_windows=wf_windows)
        ...
        # Each completed row triggers one write_replay_row call
        assert mock_loader.write_replay_row.call_count == len(completed)
    ...
    def test_apply_write_row_shape(self):
        ...
        mock_loader = _make_mock_db_loader()
        mock_loader.write_replay_row.side_effect = lambda row: written_rows.append(row)
    ...
    def test_apply_false_rows_not_written(self):
        """Failed rows are never written even under --apply."""
        mock_loader = _make_mock_db_loader(daily_pnls=[])  # Empty pnls → skipped_no_data
...
class TestGovernanceLabels:
    ...
    def test_governance_labels_on_failure(self):
        """Failed results also carry correct governance labels."""
        r = _run_single_replay(
            backtest_id=FAKE_BACKTEST_ID,
...
class TestSchemaVersion:
    ...
    def test_schema_version_stamped_on_failure(self):
        """Failed result also carries schema_version=v1_challenger."""
        r = _run_single_replay(
            backtest_id=FAKE_BACKTEST_ID,
            daily_pnls=[],
            stored_classical_ruin=None,
...
class TestAerSimulatorDefault:
    def test_backend_aer_in_result(self):
    ...
    def test_no_cloud_config_passed_to_quantum_mc(self):
        ...
        with patch("src.engine.replay.quantum_replay.run_quantum_breach_estimation") as mock_fn:
            ...
            mock_result.raw_result = {"method": "iae", "classical_fallback": False, "schema_version": "v1_challenger"}
        ...
        # cloud_config must never appear in the call kwargs
        call_kwargs = mock_fn.call_args[1] if mock_fn.call_args.kwargs else {}
        call_args = mock_fn.call_args[0] if mock_fn.call_args.args else ()
        # cloud_config is positional arg 6+ — verify it was not passed or was None
...
class TestQubitCap:
    def test_qubit_cap_10_respected(self):
        """IAE never exceeds MAX_IAE_QUBITS=10 qubits."""
        assert MAX_IAE_QUBITS == 10

    def test_large_pnl_array_triggers_bin_capping(self):
        """A very long pnl array (would need > 10 qubits) gets bins capped to 2^10."""
        # 2000 daily pnls would produce sqrt(2000)~45 bins → log2(45)~5.5 qubits — fine
        # but if n_bins was uncapped at 1024, it would need 10 qubits exactly
        # Test that the result's raw_result records the qubit_cap_applied flag
        ...
        assert r.status == "completed"
        # num_qubits should never exceed MAX_IAE_QUBITS
        assert r.num_qubits <= MAX_IAE_QUBITS, (
            f"num_qubits={r.num_qubits} exceeded cap={MAX_IAE_QUBITS}"
        )

    def test_num_qubits_never_exceeds_cap(self):
        """Completed result.num_qubits <= MAX_IAE_QUBITS always."""
        r = _run_single_replay(
            backtest_id=FAKE_BACKTEST_ID,
            daily_pnls=SYNTHETIC_DAILY_PNLS,
            stored_classical_ruin=None,
...
class TestIAEFailureHandling:
    def test_iae_failure_returns_failed_status_not_exception(self):
        ...
        assert "RuntimeError" in r.failure_reason or "IAE" in r.failure_reason or "circuit" in r.failure_reason.lower()
...
class TestReplayOnBacktest:
    ...
    def test_returns_breach_and_ruin_events(self):
        ...
        mock_loader = _make_mock_db_loader(wf_windows=[
            {"id": FAKE_FOLD_ID, "window_index": 0, "is_start": "2024-01-01", "is_end": "2024-06-30"}
        ])
        with _patch_db(mock_loader):
            results = replay_quantum_on_backtest(
    ...
    def test_no_server_imports(self):
        ...
        assert "from src.server" not in src, "Authority boundary violated: src.server imported"
        assert "import src.server" not in src, "Authority boundary violated: src.server imported"
