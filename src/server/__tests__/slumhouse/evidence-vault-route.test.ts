...
describe("Slumhouse evidence vault route", () => {
  beforeEach(() => mocks.assemble.mockReset());
  ...
  it("passes exact video and bounded search inputs into the real assembler", async () => {
    ...
    const res = response();
    await getEvidenceVault({ query: { video: "dQw4w9WgXcQ", q: "opening range" } } as any, res as any);
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe(payload);
    expect(mocks.assemble).toHaveBeenCalledWith({ videoId: "dQw4w9WgXcQ", search: "opening range" });
  });
  ...
  it("returns an explicit unavailable state instead of a false quiet archive", async () => {
    ...
    const res = response();
    await getEvidenceVault({ query: {} } as any, res as any);
