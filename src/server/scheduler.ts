...
export function initScheduler(bootCorrelationId: string | null = null) {
  ...
  // Every 12h (00:00 + 12:00 UTC) — matches the registerJob(...,12h) cadence above. The freshness
  // heartbeat re-stamps complianceRulesets.retrievedAt and MUST fire well within the 24h freshness
  // window (compliance_gate.py RULESET_MAX_AGE_HOURS) or the compliance gate goes stale and
  // hard-rejects ALL fills. Was "0 4,5 * * 0" (Sunday-only + midnight-ET guard) — a weekly heartbeat
  // cannot hold a 24h window, so the gate self-tripped mid-week (deep-scan A-1 2026-07-11, the
  // registerJob(...,12h) cadence was never actually wired — registerJob only stores metadata, the
  // scheduleUtc cron is the real trigger). 12h gives a 2x safety margin.
  scheduleUtc("0 */12 * * *", async () => {
    if (!_tryAcquireJobLock("compliance-rule-drift")) return;
    try {
    if (!(await pipelineGate("compliance-rule-drift"))) return;
    logger.info("Scheduler: Compliance rule drift check (every 12h)");
