...
export async function buildExitBarContext(bar: Bar): Promise<StyleExitBarContext | undefined> {
  try {
    ...
    if (buf.length >= 14) {
      ...
      // F3: delegate to computeMedianTrueRangeFromWindow which uses Wilder's true-range
      // (includes overnight gap via prevClose). See function comment for details.
      medianAtr14Val = computeMedianTrueRangeFromWindow(window);
    }
