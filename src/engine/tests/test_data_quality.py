...
def _clean_bars() -> pl.DataFrame:
    """Return 5 clean ES-like 5-min bars with no issues."""
    base_ts = datetime(2025, 3, 10, 9, 30, 0)
    rows = []
    for i in range(5):
        ts = datetime(
            base_ts.year, base_ts.month, base_ts.day,
            base_ts.hour, base_ts.minute + i * 5, 0,
        )
        rows.append({
            "ts_event": ts,
            "open": 5100.0 + i,
...
def test_duplicate_timestamps_detected():
    ...
    df = _make_bars(rows)
    report = validate_bars(df, "ES", "5min")
...
def test_zero_volume_flagged():
    ...
    # Zero volume alone does NOT fail the report
