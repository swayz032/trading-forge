...
PARITY — THE CLAIM, ITS ENFORCER, AND ITS HISTORY (corrected 2026-07-28, R-315 §4).

Parity is enforced by `src/engine/tests/test_session_windows_parity.py`, which
compares THIS module's five killzone booleans against answers generated by
importing the REAL `src/server/lib/killzone.ts` — 5,826 UTC instants spanning
four ET days (an ordinary EST day, an ordinary EDT day, and BOTH DST transition
days), plus second-level probes either side of all 18 declared boundaries.
First measured result: 40,782 comparisons, ZERO mismatches. The comparison is
red-proved in that file: shifting a single boundary constant by one minute makes
it fail.

★ THIS PARAGRAPH PREVIOUSLY CITED THAT TEST WHEN IT DID NOT EXIST. No file of
that name was in the repo and no cross-runtime test existed under any other name
— `test_session_windows.py` holds 16 tests, all single-runtime. The de-
approximation stated above ("NOT flagged approximation=True") was therefore
resting on a guarantee whose named enforcer was absent. The claim has now been
MEASURED and the enforcer BUILT rather than the claim withdrawn, because the
measurement came back clean. The correction is recorded rather than quietly
deleted so the next reader knows this was checked, not assumed.

Note also that the boundary CONSTANTS are not TS-derived in origin: killzone.ts
declares them as "ICT vocabulary — operator-canonical per CLAUDE.md", so the
audited windows have an authority independent of either implementation, and each
side is checkable against it. Parity is the check that the two implementations
of that authority agree; it is not itself the source of the values.
"""
