...
ARCHITECTURE (pre-reg §1): the FRAME is the unit, not the sentence.
  ...
  STEP 2  CLASSIFY -- output {gate-strength | context | cannot-determine},
      ...
      MARGIN -- is routed to the Gemma-local LLM (`gemma4:e4b-it-qat`, CLAUDE.md
      §15) WITH its frame + the sibling rules/exclusions of the same concept as
      cross-reference binding. The LLM may return `cannot-determine`.
...
tier-3; its RATE is the economics number. Confident-but-wrong is caught by the
control gate (pre-reg §3), NOT rewarded.
...
# PASS-2 calibration (2026-07-12): the label is no longer picked directly by the
# model. Pass-1 FORCE-DETERMINED (0.0 cannot-determine across 112 items) because a
# small model at temp 0.1 will always choose one of two definite labels when the
# decline path is a soft escape hatch. Instead the model reports a `clause_kind`
# (its BINDING judgment, the pre-reg §1 discriminator) and the label is DERIVED in
# code. The decline path now fires MECHANICALLY: a clause the model judges
# `ambiguous` (rule-vs-narration undecidable, or a fragment too short to bind), or
# one it claims `instantiates_stated_rule` but cannot name the rule it enacts,
# becomes `cannot-determine`. This encodes the pre-reg IFF literally:
#   narrated action = gate IFF it instantiates a rule stated elsewhere (cited);
#   directional statement = gate IFF it binds to an action/exclusion.
# Binding that cannot be located is a decline, not a guess.
_CLAUSE_KINDS = (
...
def _derive_label(clause_kind: Optional[str], bound_rule: str) -> tuple:
    ...
    so the decline path fires mechanically instead of relying on the model to
    volunteer 'cannot-determine'). Returns (label, binds_to_rule)."""
...
def classify_item(
    ...
    # --- margin -> LLM with frame + cross-reference binding ---
