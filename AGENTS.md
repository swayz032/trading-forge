...
After every parallel-subagent dispatch that returns GREEN (all tracks pass tests + CI gates), parent claude MUST `git add -A && git commit -m "..." --no-verify && git push origin <current-branch>` BEFORE dispatching the next pass. This rule was created in response to the 2026-05-19 86-file null-byte corruption incident, which wiped weeks of uncommitted Wave 21/22/23 work in 3 seconds during heavy parallel-subagent activity. The single point that would have prevented it: commit-and-push immediately after each GREEN dispatch, instead of batching commits at end-of-session. Disk failures are not predictable; commit-and-push is. Canonical spec lives in CLAUDE.md §11a — including when-to-commit / when-not-to-commit / commit-message-format. Skipping commit-and-push is **fail-CLOSED**, same severity as skipping `system-map:sync`. **`git add -A` is safe ONLY in an isolated worktree (see next) — on a shared tree, commit explicit paths (`git commit -o <paths>`).**

### Forcing Function: Multi-Session Worktree Isolation (HARD RULE, pinned 2026-07-05)

When two or more Claude sessions/agents work this repo concurrently, each MUST operate in its OWN `git worktree` and integrate to `hardening/phase-0` → `main` only at the END — never edit/commit concurrently in one shared working directory. A shared checkout means a shared `.git` index + `HEAD` + `refs/stash`; that shared state caused the 2026-05-19 corruption, cross-session `git add -A` file sweeps, branch-switch HEAD moves, and stash clobbers. Safe protocol (canonical spec in CLAUDE.md §11b): own dir per session; base pinned to an explicit SHA not a branch name (a branch base tracks the moving shared HEAD — the Deep-Scan #16 wrong-reseed hazard); NEVER `git stash` in a worktree (shared ref); verify tsc + tests + 3 CI gates GREEN in the worktree; land FF-only; clean up with `git worktree remove`. The Agent tool's `isolation:"worktree"` forks from the shared HEAD at spawn time, so the SHA-pin hazard still applies. Fallback when isolation is genuinely impossible: explicit-path commits only, `git status -sb` before every push. Violating isolation on a live multi-session repo is **fail-CLOSED** — it risks silent data loss, not just drift.
