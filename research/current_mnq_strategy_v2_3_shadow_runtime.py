#!/usr/bin/env python3
"""Personal-device shadow runtime for Current MNQ v2.3.

This is the production signal path without order submission. It continuously
proves: realtime dual-hub health, simulated-account eligibility, REST/realtime
balance agreement, broker flatness, causal completed-bar signal generation,
first-A+ chronology, and end-of-day replay parity.
"""
...
from research.current_mnq_strategy_v2_3_engine import Params, load_production_dataset
...
from research.current_mnq_strategy_v2_3_signal import (
...
class ShadowRuntime:
    def __init__(self, *, account_id: int, realtime_snapshot_path: str | Path,
        ...
        require_personal_device("MNQ_SHADOW_RUNTIME")
    ...
    def _ensure_context(self, session: date, now_utc: datetime) -> dict:
        if not self.context.manifest_path.exists():
            manifest = self.context.bootstrap(
                session, lookback_days=BOOTSTRAP_LOOKBACK_DAYS, as_of_utc=now_utc
            )
            self._last_context_refresh = pd.Timestamp(now_utc)
            return manifest
        now_ts = pd.Timestamp(now_utc)
        due = (
            self._last_context_refresh is None or
            (now_ts - self._last_context_refresh).total_seconds() >= CONTEXT_REFRESH_SECONDS
        )
        if due:
    ...
    def step(self, now: datetime | None = None) -> dict:
        ...
        account, orders, positions = self._rest_state(rt.account_balance)
        position = sum(
        ...
        heartbeat_due = (
            ...
            (local - self._last_heartbeat).total_seconds() >= HEARTBEAT_SECONDS - 1.0
        )
        if heartbeat_due:
            self.journal.record_heartbeat(
                session, contract_id=contract_id, account_simulated=rt.account_simulated,
                feed_age_seconds=rt.feed_age_seconds,
                user_hub_connected=rt.user_hub_connected,
                market_hub_connected=rt.market_hub_connected,
                broker_position=position, working_orders=len(orders),
                best_bid=rt.best_bid, best_ask=rt.best_ask,
            )
        ...
        if orders or positions:
            return {
                "status": "BROKER_STATE_NOT_FLAT", "session": session,
        ...
        if not signal_is_fresh(decision, local):
            self.journal.record_snapshot(
                session, would_trade=False, decision=payload,
                contract_id=contract_id, account_simulated=rt.account_simulated,
                feed_age_seconds=rt.feed_age_seconds,
                user_hub_connected=rt.user_hub_connected,
                market_hub_connected=rt.market_hub_connected,
                broker_position=0, working_orders=0,
                best_bid=rt.best_bid, best_ask=rt.best_ask,
        ...
        self.journal.record_snapshot(
            session, would_trade=True, decision=payload,
            contract_id=contract_id, account_simulated=rt.account_simulated,
            feed_age_seconds=rt.feed_age_seconds,
            user_hub_connected=rt.user_hub_connected,
            market_hub_connected=rt.market_hub_connected,
    ...
    def replay_session(self, session_date: date) -> dict:
        """End-of-day replay of the exact first-A+ causal question."""
        session = str(session_date)
        events = [r for r in read_events(self.journal.path) if str(r.get("session")) == session]
        live_decisions = [r for r in events if r.get("event_type") == "DECISION"]
        original = next(
            (r for r in live_decisions if r.get("would_trade") or r.get("note") == MISSED_FIRST_NOTE),
            None,
        )

        # Use 12:00 ET for a no-trade day. For a live/missed setup replay at the
        # exact decision timestamp and feed the recorded BBO back into the kernel.
        if original is None:
            as_of = pd.Timestamp(f"{session} 12:00", tz=TZ)
            bid = ask = None
        else:
            as_of = pd.Timestamp(original["timestamp_utc"]).tz_convert(TZ)
            bid = original.get("best_bid")
            ask = original.get("best_ask")

        raw5, raw1, manifest = load_production_dataset(self.context_root)
        env = prepare_causal(raw5, raw1, manifest, as_of)
        replay = find_first_actionable_signal(
            env, session_date, Params(), as_of,
        ...
        self.journal.record_replay_parity(
            session, live_setup_fp, replay_setup_fp, live_exec_fp, replay_exec_fp
        )
        return {
            "session": session,
            "live_decision": live_payload,
            "replay_decision": replay_payload,
    ...
    def run_until_window_end(self, poll_seconds: float = 1.0) -> None:
        """Foreground shadow loop; intended to run on the user's personal device."""
        while True:
            local = _local_now()
            self.step(local.to_pydatetime())
            if local.time() > EXECUTION_END:
                self.replay_session(local.date())
                return
