...
#: ALGO-175. THE ONLY LINE THAT DIFFERS FROM run_algo141_what_drops_his_setup_after_ranking.py.
#: This is the SAME walk re-run against the REPAIRED kernel; the walk logic, manifest, data
#: lock and gates are untouched, so any difference is attributable to the kernel repair and to
#: nothing this script did. It writes to a NEW FILE ON PURPOSE - the algo141 artifact is the
#: PRE-REPAIR baseline ALGO-173 was measured on, and overwriting it would destroy the only
#: record of the defect while claiming to have fixed it.
OUT = Path("research/current_mnq_strategy_v2_4_algo175_postrepair_walk.json")
