#!/usr/bin/env python3
"""Personal-device one-trade automation runtime for MNQ v2.3.

This is intentionally small. Signal semantics live in the causal signal kernel;
execution semantics live in the fail-closed broker. This loop only composes them
without inventing fallback trades.
"""
...
from research.current_mnq_strategy_v2_3_broker import V23ProjectXBroker
from research.current_mnq_strategy_v2_3_engine import Params, load_production_dataset
from research.current_mnq_strategy_v2_3_execution import submit_with_realtime_snapshot
...
class AutomationRuntime:
    def __init__(self, *, account_id: int,
                 ...
                 broker: V23ProjectXBroker | None = None,
                 context: LiveContextStore | None = None):
        require_personal_device("MNQ_V23_AUTOMATION_RUNTIME")
        ...
        if risk_store.config.account_id != self.account_id:
            raise RuntimeError("AUTOMATION_RISK_ACCOUNT_MISMATCH")
        self.receipt = Path(promotion_receipt)
        self.broker = broker or V23ProjectXBroker(account_id=self.account_id)
    ...
    def evaluate_once(self, now: datetime | None = None,
        ...
        if not self.context.manifest_path.exists():
            self.context.bootstrap(
                session_date, lookback_days=100,
                as_of_utc=local.tz_convert("UTC").to_pydatetime(),
            )
        else:
            self.context.refresh(
                session_date, as_of_utc=local.tz_convert("UTC").to_pydatetime()
            )
        ...
        if not signal_is_fresh(decision, local):
            ...
            return {
                "status": "MISSED_FIRST_A_PLUS_SIGNAL_DAY_DISABLED",
                "session": session,
        ...
        result = submit_with_realtime_snapshot(
            broker=self.broker,
            signal=_broker_signal(decision),
