...
for (const q of QUERIES) {
  const r = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(q)}&type=video&maxResults=50&order=relevance&videoCaption=closedCaption&videoDuration=medium&relevanceLanguage=en&key=${YT_KEY}`);
