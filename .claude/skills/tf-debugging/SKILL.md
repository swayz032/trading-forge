...
| A16. DB `ECONNRESET` (any tool: drizzle, `postgres.js`, a script) | "credentials rotated" / "password is stale" / retry the DSN | **TWO Railway projects exist and differ by ONE SPACE: `trading-forge` (RETIRED, all services dead) vs `Trading Forge` (LIVE, service `Postgres-KcfX` @ `sakura.proxy.rlwy.net:34357`).** A **project token is scoped to ONE project**, so a valid token opens the retired one and `railway status` cheerfully confirms a project name — **the wrong one**. ⇒ **DIAGNOSE FIRST, in this order:** (1) **zero-credential Postgres `SSLRequest` probe** — send 8 bytes (`int32 8`, `int32 80877103`) and read the reply: `"S"` = backend ALIVE, reset = backend ABSENT. **No credentials are sent, so password, auth and client library are excluded in ONE probe.** Run it against BOTH hosts so each is the other's control. (2) `railway status --json` → per-service `latestDeployment`/`activeDeployments`. 🛑 **A bare TCP connect is NOT a liveness test — a retired Railway proxy still ACCEPTS TCP at the edge while routing nowhere**, so `net.connect()` succeeds on the dead host. ⇒ **`WHEN A CONNECTION DIES BEFORE YOU SEND CREDENTIALS, IT IS NOT A CREDENTIAL PROBLEM.`** ★ **`AUTHENTICATING SUCCESSFULLY IS NOT AUTHENTICATING TO THE RIGHT PLACE` — confirm the tool printed the project you MEANT.** (`railway list` returns `Unauthorized` under a project token — that is normal, not a broken token; verify with `railway status`, never `whoami`.) Detail + repair record: memory `ops_db_moved_to_new_railway_project_env_is_stale`. |
