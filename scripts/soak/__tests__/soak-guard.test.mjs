...
const base = {
  sample: { health: { reachable: true, ok: true, backtestsActive: 0 }, gpuUtil: 5, pythonCount: 0 },
...
test("backend UP but auth-gated/slow (reachable:true, ok:false) → NOT unreachable → RUN (the 07-11 false-positive fix)", () => assert.equal(G.decide({ ...base, sample: { health: { reachable: true, ok: false, backtestsActive: null }, gpuUtil: 5, pythonCount: 0 } }).action, "RUN"));
